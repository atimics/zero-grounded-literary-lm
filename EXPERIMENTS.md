...
- **Latest mechanics outcome**: Sero Latent v3 completed its frozen 100M-byte, three-seed AWS run on the promoted 123,153,182-byte training corpus
- **Evaluation decision**: Do not promote V3; mean latent quality was 14.53% worse than byte-BPE, all seeds failed quality, and two seeds also failed compute parity
- **Next training experiment**: Train the actual Sero language-model baseline on `sero-pretrain/2026-08-22.v1` with the locked 4,096-entry lossless byte-BPE tokenizer
