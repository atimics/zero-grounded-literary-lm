...
| **q23** | `benchmarks/zero4-q23-v1` | 2026-07-19 | `ZERO4-BACKLOG.md` P0–P4 | Preregistered checkpoint-v4 transactional AdamW, per-attempt faculty/replay diagnostics, exact learned-state rollback, observer-derived guard calibration, and seed/promotion sealing. Student, teachers, corpora, routing, and public thresholds remain fixed from Q2.2. | Can optimizer-boundary observation and rollback prevent replay interference without weakening the quantity or 2% replay gates? | **Not executed.** Mechanics, resume equivalence, and a reduced observer rehearsal pass locally; the full observer trajectory is still unopened. | **Preregistered only.** No capability result and no promotion claim. Guarded diagnostic seed 2 remains blocked on merged CI and the full byte-identical observer gate. | Merge implementation, then run full observer seed 2; run guarded seed 2 only if observer passes |
