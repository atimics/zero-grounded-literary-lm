...
| Shared-state bottleneck | 152-wide verified-factor head with a continuous answer bridge | C6.1 terminal no-go; state learning and retention passed, while retrieval and bridge-contribution gates failed |
| Hierarchical tokenization studies | merge-tree embeddings, fixed-block recurrent state, and a prompt-predicted answer root | terminal C6.1 result bound; HT1 is ready for implementation review |
...
C6.1 reached a terminal no-go result. Its auxiliary state head learned, while
the retrieval, paired-choice, and bridge-contribution gates failed. The result
and selected checkpoint are hash-bound. Retention passed and the sealed test
stayed closed.

Three hierarchical tokenization studies are frozen as a separate series. HT1
