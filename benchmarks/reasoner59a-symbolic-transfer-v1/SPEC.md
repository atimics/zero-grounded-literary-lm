...
replay. The checker re-executes every raw row from the manifest and source
artifact before it rebuilds the result. The common gate requires all four
semantic shifts, both transfer directions, and both support builders. The
one-sided primary error allocation is 0.005.
