...
AVX512_LINEAR_CFLAGS ?= -mavx512f -mfma -DUSE_AVX512_LINEAR
...
.PHONY: all check clean web channel-data zero3-data zero3-stage1 \
	...
	zero5-avx512-linear-check zero5-avx512-linear-aws-check \
	zero5-avx512-linear-aws-result-check \
	...
	zero5-c42-check zero5-c42-aws-check zero5-c42-run \
...
zero5_c32_lm_avx512_linear: zero5_c32_lm.c channel_protocol.h zero1_protocol.h
	$(CC) $(CFLAGS) $(CPU_FAST_CFLAGS) $(GNU_LIBMVEC_CFLAGS) \
		$(AVX512_LINEAR_CFLAGS) -DUSE_LIBMVEC_TANH -DUSE_LIBMVEC_EXP \
		$(ZERO5_THREAD_FLAGS) $(LITERARY_CFLAGS) zero5_c32_lm.c -o $@ \
		$(LITERARY_LDLIBS) $(ZERO5_THREAD_FLAGS)
...
zero5-avx512-linear-check:
	node scripts/benchmark_zero5_avx512_linear.mjs --self-test
	@if test "$(UNAME_S)-$(UNAME_M)" = "Linux-x86_64"; then \
		$(MAKE) zero5_c32_lm_vector_math zero5_c32_lm_avx512_linear; \
		if grep -qw avx512f /proc/cpuinfo && grep -qw fma /proc/cpuinfo; then \
			./zero5_c32_lm_avx512_linear --self-test && \
			ZERO5_VECTOR_BINARY=./zero5_c32_lm_vector_math \
				ZERO5_LINEAR_BINARY=./zero5_c32_lm_avx512_linear \
				node scripts/check_zero5_c32.mjs; \
		else \
			echo "AVX-512 execution skipped: CPU feature unavailable"; \
		fi; \
	else \
		echo "AVX-512 trainer check skipped: requires Linux x86-64"; \
	fi

zero5-avx512-linear-aws-check:
	bash -n scripts/aws/zero5-avx512-linear-stage.sh
	bash -n scripts/aws/zero5-avx512-linear-run-instance.sh
	bash -n scripts/aws/zero5-avx512-linear-user-data.sh
	node scripts/check_zero5_avx512_linear_aws.mjs

zero5-avx512-linear-aws-result-check: zero5-avx512-linear-aws-check
	node scripts/check_zero5_avx512_linear_aws_result.mjs
...
check: sero-series-closure-check zero5-c0-check zero5-c1-check zero5-c2-check zero5-c3-check zero5-c31-check zero5-c32-check zero5-c33-check zero5-c33-parallel-check zero5-c33-parallel-result-check zero5-c42-check zero5-c42-aws-check zero5-cpu-profile-check zero5-cpu-profile-aws-check zero5-cpu-profile-aws-result-check zero5-vector-math-check zero5-vector-math-aws-check zero5-vector-math-aws-result-check zero5-blocked-attention-check zero5-avx512-linear-check zero5-avx512-linear-aws-check zero5-avx512-linear-aws-result-check zero5-tensor-batch-check zero5-tensor-aws-check zero5-tensor-aws-result-check
...
clean:
	rm -f zero_lm literary_lm zero5_lm zero5_c2_lm zero5_c3_lm zero5_c31_lm zero5_c32_lm zero5_c32_lm_fast zero5_c32_lm_profile zero5_c32_lm_vector_tanh zero5_c32_lm_vector_exp zero5_c32_lm_vector_math zero5_c32_lm_attention_b32 zero5_c32_lm_attention_b64 zero5_c32_lm_attention_b128 zero5_c32_lm_qkv_forward zero5_c32_lm_qkv_backward zero5_c32_lm_qkv zero5_c32_lm_tensor zero5_c32_lm_tensor_qkv zero5_braid bpe_tokenizer sero_tokenizer logic_corpus brainfuck_corpus channel_corpus faculty_controller export_literary freeze_literary_teacher literary_infer memorization_eval zero_eval faculty_eval quantity_request_eval external_eval quantity_adapter_pilot package_quantity_adapter quantity_adapter_infer quantity_adapter_request_eval base_probability_infer operation_head_pilot package_operation_head operation_head_infer operation_head_request_eval runtime_operation_head_pilot package_runtime_operation_head semantic_operation_eval semantic_runtime_head_pilot
