...
.PHONY: all check clean web channel-data zero3-data zero3-stage1 \
	...
	zero5-c61-terminal-check zero5-c61-evaluator-preflight-check \
	zero5-hierarchical-tokenization-check zero5-ht1-authorization-check \
	zero5-ht1-aws-check \
...
all: reasoner0 reasoner1 reasoner2 reasoner3 reasoner31 reasoner32 reasoner33 \
	...
	reasoner50 reasoner51 reasoner52 \
	weight_multiplicity
...
.PHONY: reasoner0-check reasoner1-check reasoner2-check reasoner3-check \
	...
	reasoner51-check reasoner51-contract-check \
	reasoner52-check reasoner52-contract-check \
...
reasoner51-check: reasoner51
	./reasoner51 --self-test
	@if ./reasoner51 execute /tmp/reasoner51-unauthorized-result.json \
		/tmp/reasoner51-unauthorized-execution.json \
		/tmp/reasoner51-unauthorized-artifact.bin >/dev/null 2>&1; then \
		echo "Reasoner 5.1 scientific execution unexpectedly opened"; exit 1; \
	fi

reasoner50-result-check:
	node scripts/check_reasoner50_result.mjs

reasoner52-contract-check:
	node scripts/check_reasoner52_contract.mjs

.PHONY: reasoner52-result-check
reasoner52-result-check:
	node scripts/check_reasoner52_result.mjs

check: reasoner52-result-check

reasoner52: reasoner52.c reasoner52.h reasoner52_cli.c
	$(CC) $(CFLAGS) reasoner52.c reasoner52_cli.c -o $@

reasoner52-check: reasoner52
	./reasoner52 --self-test
	@if ./reasoner52 execute /tmp/reasoner52-unauthorized-result.json \
		/tmp/reasoner52-unauthorized-execution.json \
		/tmp/reasoner52-unauthorized-artifact.bin >/dev/null 2>&1; then \
		echo "Reasoner 5.2 scientific execution unexpectedly opened"; exit 1; \
	fi
...
zero5-c61-terminal-check: zero5-c61-evaluation-recovery-check
	node scripts/check_zero5_c61_terminal.mjs

zero5-c61-evaluator-preflight-check: zero5-c61-terminal-check
	node scripts/check_zero5_c61_evaluator_preflight.mjs

zero5-hierarchical-tokenization-check:
	node scripts/check_zero5_hierarchical_tokenization.mjs

.PHONY: zero5-ht1-authorization-check
zero5-ht1-authorization-check: zero5-hierarchical-tokenization-check
	node scripts/check_zero5_ht1_authorization.mjs

.PHONY: zero5-ht1-aws-check
zero5-ht1-aws-check: zero5-ht1-authorization-check
	node scripts/check_zero5_ht1_aws.mjs

.PHONY: zero5-ht1-mergetree-check
check: zero5-ht1-mergetree-check
zero5-ht1-mergetree-check: zero5_ht1_mergetree_lm zero5_c32_lm_vector_math sero_tokenizer
	node scripts/check_zero5_ht1_mergetree.mjs
...
zero5_ht1_mergetree_lm: zero5_ht1_mergetree_lm.c tests/zero5_ht1_mergetree_test.h zero5_c32_lm.c channel_protocol.h zero1_protocol.h
	$(CC) $(CFLAGS) $(CPU_FAST_CFLAGS) $(GNU_LIBMVEC_CFLAGS) \
		-DUSE_LIBMVEC_TANH -DUSE_LIBMVEC_EXP $(ZERO5_THREAD_FLAGS) \
		$(LITERARY_CFLAGS) zero5_ht1_mergetree_lm.c -o $@ \
		$(LITERARY_LDLIBS) $(ZERO5_THREAD_FLAGS)
