                            ...
                            const Config *cfg)
{
    ...
    if (version != 1U || set->vocab != (uint32_t)cfg->vocab ||
    ...
    for (index = 0; index < class_total; ++index) {
        ...
        if (value > 2U) fail("packed target class exceeds 2");
        if (value != 0U) ++observed_active;
        if (value == 2U) ++observed_answers;
    }
    if (observed_active != set->active_targets ||
        observed_answers != set->answer_targets || fgetc(file) != EOF) {
...
static uint64_t packed_mask(const PackedSet *set, uint32_t pack,
                            unsigned char *mask, int answer_weight)
{
    ...
    for (time = 0; time < set->context; ++time) {
        ...
        mask[time] = value == 2U ? (unsigned char)answer_weight : value;
        if (value != 0U) ++active;
    }
    return active;
}
...
{
    unsigned char *mask = zero_alloc((size_t)set->context, 1);
    double total_loss = 0.0;
    ...
    for (sample = 0; sample < batches; ++sample) {
        ...
        uint64_t active = packed_mask(set, pack, mask, 1);
        float loss;
...
{
    unsigned char *mask = zero_alloc((size_t)train->context, 1);
    ...
    while (completed_steps < (uint64_t)options->steps && !interrupted) {
        ...
        for (batch = 0; batch < options->batch; ++batch) {
            ...
            uint64_t active = packed_mask(train, pack, mask,
                                          options->answer_weight);
    ...
    printf("packed sampling sequences=%u compute-token-exposures=%llu "
           "active-targets=%llu answer-targets=%llu padding-targets=%llu "
           "wraps=0 answer-weight=%d\n",
           train->pack_count,
...
static void print_usage(const char *program)
{
    ...
    printf("  --packed-train F     consume one record-safe Z5PKV1 pack set exactly once\n");
    printf("  --packed-validation F use a record-safe Z5PKV1 validation pack set\n");
    printf("  --answer-weight N    packed answer-token weight, 1..32 (default: 1)\n");
...
int main(int argc, char **argv)
{
    ...
    for (i = 1; i < argc; ++i) {
        ...
        } else if (strcmp(argv[i], "--answer-weight") == 0 &&
                   i + 1 < argc) {
            options.answer_weight =
                (int)parse_long(argv[++i], "--answer-weight");
    ...
    if (options.completion_eval_path != NULL) {
        ...
        options.generate_tokens = 0;
        if (options.resume_path == NULL && options.init_path == NULL) {
            fail("--completion-eval requires --resume or --init");
        }
        if (options.eval_only) {
            fail("--completion-eval and --eval-only are mutually exclusive");
        }
    }
    ...
    if (options.steps < 0 || options.batch < 1 || options.learning_rate < 0.0f ||
        ...
        options.answer_weight < 1 || options.answer_weight > 32 ||
