...
int main(int argc, char **argv)
{
    ...
    Q31CheckpointHeader checkpoint;
    ...
    float *values;
    if (argc != 4) {
        fprintf(stderr, "usage: %s BASE_LITQ8 HEAD_Q31 OUTPUT\n", argv[0]);
        return EXIT_FAILURE;
    }
    ...
    read_exact(head, &checkpoint, sizeof(checkpoint), 1, head_path);
    if (memcmp(checkpoint.magic, checkpoint_magic, 8) != 0 ||
        ...
        fail("operation-head checkpoint does not match base model");
    ...
    read_exact(head, values, sizeof(float), weights + checkpoint.classes,
               head_path);
    if (fseek(head,
              (long)(2 * (weights + checkpoint.classes) * sizeof(float)),
              SEEK_CUR) != 0 || fgetc(head) != EOF)
        fail("operation-head checkpoint optimizer state is corrupt");
    ...
    memcpy(package.magic, package_magic, sizeof(package.magic));
    package.version = 1;
    package.base_length = (uint32_t)base_length;
    package.base_padded_length = (uint32_t)padded;
    package.vocab = base_header.vocab;
    package.context = base_header.context;
    package.dim = base_header.dim;
    package.heads = base_header.heads;
    package.layers = base_header.layers;
    package.ff = base_header.ff;
    package.classes = checkpoint.classes;
    package.feature_dim = checkpoint.feature_dim;
    package.step = checkpoint.step;
    ...
    write_exact(output, values, sizeof(float),
                weights + checkpoint.classes, temporary);
    if (fclose(output) != 0 || rename(temporary, output_path) != 0)
        fail_path("install", output_path);
    free(values); free(temporary); free(base);
    printf("packaged routed operation head at update %llu to %s\n",
