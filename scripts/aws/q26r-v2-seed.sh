#!/bin/bash
# Run one frozen Q2.6-R replacement seed inside its independently bounded AWS instance.
...
STATUS_FILE="/tmp/zero-q26r-v2-seed${ZERO_SEED}-status.json"
WORKLOAD_LOG="/var/log/zero-q26r-v2-seed${ZERO_SEED}.log"
BACKEND_LOG="/tmp/zero-q26r-v2-seed${ZERO_SEED}-backend.log"
...
write_fallback_status() {
  ...
  ZERO_EXIT_CODE="$exit_code" ZERO_FINISHED_AT="$finished_at" \
    ZERO_FINISHED_EPOCH="$finished_epoch" ZERO_PHASE="$PHASE" \
    ZERO_RUN_STATUS="$run_status" ZERO_STARTED_AT="$STARTED_AT" \
    python3 - <<'PY'
import json
import os
...
finish() {
  ...
  aws s3 cp "$WORKLOAD_LOG" \
    "s3://${ZERO_BUCKET}/jobs/${ZERO_RUN_ID}/seed${ZERO_SEED}/zero-q26r-v2-seed${ZERO_SEED}.log" \
    --no-cli-pager || publication_exit=$?
  # Status is the collector's immutable completion signal. Publish it last and
  # refuse replacement so a duplicate worker cannot rewrite the decision.
...
record = {
    "schema": "zero.aws_q26r_seed_heartbeat.v2",
    "experiment": "zero4-q26r-aws-v2",
    "seed": int(os.environ["ZERO_SEED"]),
    "instance_id": os.environ["ZERO_INSTANCE_ID"],
    "status": "running",
...
with open("/tmp/zero-q26r-v2-heartbeat.json", "w", encoding="utf-8") as handle:
    json.dump(record, handle, indent=2)
    handle.write("\n")
PY
  aws s3 cp /tmp/zero-q26r-v2-heartbeat.json \
...
echo "Q2.6-R AWS v2 seed $ZERO_SEED completed with decision $decision"
