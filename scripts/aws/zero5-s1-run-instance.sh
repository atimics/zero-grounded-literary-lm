...
lock_key=experiments/zero5-s1-scale-v1/execution-v4.lock
...
test "$(jq -r .authorization.approval_id benchmarks/zero5-s1-scale-v1/contract.json)" = "$ZERO5_APPROVAL_ID"
test "$(jq -r .execution.venue benchmarks/zero5-s1-scale-v1/contract.json)" = "aws us-east-1 c6i.4xlarge on-demand"
test "$(jq -r .execution.maximum_instance_seconds benchmarks/zero5-s1-scale-v1/contract.json)" = "$maximum_seconds"
test "$(jq -r .execution.maximum_ec2_usd benchmarks/zero5-s1-scale-v1/contract.json)" = "$max_usd"
test "$(jq -r .execution.spot_instances benchmarks/zero5-s1-scale-v1/contract.json)" = false
...
request=(ec2 run-instances
  ...
  --user-data file://scripts/aws/zero5-s1-user-data.sh
  ...
  --tag-specifications "$tags"
    "ResourceType=volume,Tags=[{Key=Project,Value=zero},{Key=Experiment,Value=zero5-s1-scale-v1},{Key=RunId,Value=${ZERO5_RUN_ID}}]"
...
jq -n --arg run_id "$ZERO5_RUN_ID" \
  ...
  '{schema:"zero.s1_aws_execution_lock.v1",run_id:$run_id,
...
jq -n --arg run_id "$ZERO5_RUN_ID" --arg instance_id "$instance_id" \
  ...
  --argjson maximum_seconds "$maximum_seconds" \
  --argjson maximum_usd "$max_usd" \
  '{schema:"zero.s1_aws_launch.v1",run_id:$run_id,
...
aws s3api put-object --region "$ZERO5_REGION" \
  ...
  --key "experiments/zero5-s1-scale-v1/${ZERO5_RUN_ID}/launch.json" \
