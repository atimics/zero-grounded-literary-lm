...
RESULTS_ROOT=/tmp/zero-q28-language-gate-results
RESULT_FILE="$RESULTS_ROOT/result.json"
STATUS_FILE=/tmp/zero-q28-language-gate-status.json
WORKLOAD_LOG=/var/log/zero-q28-language-gate.log
...
heartbeat() {
  jq -n --arg phase "$PHASE" --arg git_commit "$ZERO_COMMIT" \
    ...
    '{
      schema: "zero.q28_language_gate_heartbeat.v1",
      phase: $phase,
      git_commit: $git_commit,
      candidate_sha256: $candidate_sha256,
      at: $at,
      elapsed_instance_seconds: $elapsed
    }' > /tmp/zero-q28-language-gate-heartbeat.json
  aws s3 cp /tmp/zero-q28-language-gate-heartbeat.json \
...
test "$(sha256sum "$ZERO_BUDGET_FILE" | awk '{print $1}')" = \
  "$ZERO_BUDGET_SHA256"
test "$(sha256sum benchmarks/zero4-q28-v1/language-gate/candidate.litq8 | awk '{print $1}')" = \
...
timeout --signal=TERM --kill-after=10s "${remaining}s" \
  node scripts/run_zero_language_gate.mjs \
    ...
    --model benchmarks/zero4-q28-v1/language-gate/candidate.litq8 \
    --model-id zero4-q28-seed2-u200 \
