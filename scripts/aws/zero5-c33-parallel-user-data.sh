...
BOOT_LOG=/var/log/zero5-c33-parallel-bootstrap.log
...
RESULT_PREFIX="experiments/zero5-c33-parallel-v1/${RUN_ID}"
STATE_PREFIX="${RESULT_PREFIX}/state"
STATUS_FILE=/tmp/zero5-c33-parallel-status.json
TERMINAL_WRITTEN=0
PHASE=bootstrap
...
test "$(sha256sum benchmarks/zero5-c33-parallel-v1/contract.json | awk '{print $1}')" = "$CONTRACT_SHA256"
test "$(jq -r .authorized benchmarks/zero5-c33-parallel-v1/contract.json)" = true
...
make zero5_c32_lm_fast
node scripts/check_zero5_c33_parallel.mjs

OUT=build/zero5-c33-parallel-aws-v1/run
...
timeout --signal=TERM --kill-after=90s "${remaining}s" \
  node scripts/run_zero5_c33_parallel.mjs "${runner_args[@]}" &
RUNNER_PID=$!
