...
user_data=scripts/aws/zero5-avx512-linear-user-data.sh
...
request=(ec2 run-instances
  ...
  --tag-specifications "$tags"
    "ResourceType=volume,Tags=[{Key=Project,Value=zero},{Key=Experiment,Value=zero5-avx512-linear-v1},{Key=RunId,Value=${ZERO5_RUN_ID}}]"
...
if [ "$action" = dry-run ]; then
  ...
  echo "ZERO.5 AVX-512 linear AWS dry-run passed"
  exit 0
fi

lock_key=experiments/zero5-avx512-linear-v1/execution.lock
lock=$(mktemp)
jq -n --arg run_id "$ZERO5_RUN_ID" \
  ...
  '{schema:"zero.aws_avx512_linear_execution_lock.v1",run_id:$run_id,
...
jq -n --arg run_id "$ZERO5_RUN_ID" --arg instance_id "$instance_id" \
  ...
  --argjson maximum_usd "$max_usd" \
  '{schema:"zero.aws_avx512_linear_launch.v1",run_id:$run_id,
...
aws s3api put-object --region "$region" \
  ...
  --key "experiments/zero5-avx512-linear-v1/${ZERO5_RUN_ID}/launch.json" \
