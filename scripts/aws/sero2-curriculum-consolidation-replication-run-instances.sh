...
test "$SERO_RESUME_SHA256" = "$(jq -r ".initialization.checkpoint_sha256_by_seed[\"${seed}\"]" "$contract")"
[[ "$SERO_AMI" =~ ^ami-[0-9a-f]+$ ]]
...
request=(ec2 run-instances
  ...
  --tag-specifications "$tags"
    "ResourceType=volume,Tags=[{Key=Project,Value=sero},{Key=Experiment,Value=sero2-curriculum-consolidation-replication-v1},{Key=RunId,Value=${SERO_RUN_ID}}]"
...
if [ "$action" = dry-run ]; then
  ...
  echo "Sero curriculum consolidation replication seed ${seed} dry-run passed"
else
