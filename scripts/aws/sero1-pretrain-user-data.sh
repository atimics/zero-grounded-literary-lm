...
BOOT_LOG=/var/log/sero1-pretrain-bootstrap.log
...
RESULT_PREFIX="experiments/sero1-pretrain-v1/${RUN_ID}"
STATUS_FILE=/tmp/sero1-pretrain-status.json
...
write_status() {
  ...
  jq -n --arg status "$status" --arg phase "$PHASE" --arg mode "$MODE" \
    ...
    '{schema:"sero.pretrain_v1_aws_status.v1",status:$status,phase:$phase,
...
write_status running 0
upload_status
python3 -m venv build/sero1-pretrain/aws-venv
build/sero1-pretrain/aws-venv/bin/pip install --disable-pip-version-check \
  --no-input -r experiments/sero1-pretrain/requirements.txt
nvidia-smi > build/sero1-pretrain/nvidia-smi.txt
build/sero1-pretrain/aws-venv/bin/python - <<'PY'
import torch
if not torch.cuda.is_available():
    raise RuntimeError("CUDA is unavailable")
print(torch.cuda.get_device_name(0))
PY
build/sero1-pretrain/aws-venv/bin/pip freeze > build/sero1-pretrain/aws-pip-freeze.txt
build/sero1-pretrain/aws-venv/bin/python experiments/sero1-pretrain/tests.py \
...
upload_status
if [ "$MODE" = calibration ]; then
  RESULT_FILE=build/sero1-pretrain/aws-calibration/seed0.json
  ARTIFACT_DIR=build/sero1-pretrain/aws-calibration/artifacts
  TRAIN_ARGS=(--mode calibration --max-updates 128 --validation-byte-limit 131072)
else
  RESULT_FILE="benchmarks/sero1-pretrain-v1/seed${SEED}.json"
  ARTIFACT_DIR="build/sero1-pretrain/seed${SEED}"
  TRAIN_ARGS=(--mode full)
fi
...
timeout --signal=TERM --kill-after=30s "${remaining}s" \
  build/sero1-pretrain/aws-venv/bin/python experiments/sero1-pretrain/train.py \
    --manifest build/sero-pretrain-v1/manifest.json \
...
aws s3 cp build/sero1-pretrain/nvidia-smi.txt \
  "s3://${TRAINING_BUCKET}/${RESULT_PREFIX}/nvidia-smi.txt" --only-show-errors
aws s3 cp build/sero1-pretrain/aws-pip-freeze.txt \
