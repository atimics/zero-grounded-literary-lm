...
contract=benchmarks/zero5-vector-validation-v1/aws-contract.json
...
user_data=scripts/aws/zero5-vector-validation-user-data.sh

test "$region" = us-east-1
test "$instance_type" = c6i.4xlarge
test "$hourly_price" = 0.68
test "$max_seconds" = 900
test "$max_usd" = 0.17
...
request=(ec2 run-instances
  ...
  --tag-specifications "$tags"
    "ResourceType=volume,Tags=[{Key=Project,Value=zero},{Key=Experiment,Value=zero5-vector-validation-v1},{Key=RunId,Value=${ZERO5_RUN_ID}}]"
...
if [ "$action" = dry-run ]; then
  ...
  echo "ZERO.5 vector validation AWS dry-run passed"
  exit 0
fi

lock_key=experiments/zero5-vector-validation-v1/execution.lock
lock=$(mktemp)
jq -n --arg run_id "$ZERO5_RUN_ID" \
  --arg contract_sha256 "$contract_sha256" --arg approval_id "$approval_id" \
  --arg source_sha256 "$source_sha256" --arg asset_sha256 "$asset_sha256" \
  --argjson maximum_usd "$max_usd" \
  '{schema:"zero.aws_vector_validation_execution_lock.v1",run_id:$run_id,
...
jq -n --arg run_id "$ZERO5_RUN_ID" --arg instance_id "$instance_id" \
  ...
  --argjson maximum_usd "$max_usd" \
  '{schema:"zero.aws_vector_validation_launch.v1",run_id:$run_id,
...
aws s3api put-object --region "$region" \
  ...
  --key "experiments/zero5-vector-validation-v1/${ZERO5_RUN_ID}/launch.json" \
