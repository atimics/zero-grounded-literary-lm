...
request=(ec2 run-instances
  ...
  --user-data file://scripts/aws/sero-latent-v3-user-data.sh
  ...
  --tag-specifications "$tags"
    "ResourceType=volume,Tags=[{Key=Project,Value=sero},{Key=Experiment,Value=sero-latent-v3},{Key=RunId,Value=${SERO_RUN_ID}}]"
...
if [ "$action" = dry-run ]; then
  ...
  echo "Sero Latent V3 ${mode} seed ${seed} dry-run passed"
else
