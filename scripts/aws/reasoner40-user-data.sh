...
BOOT_LOG=/var/log/reasoner40-bootstrap.log
...
RESULT_PREFIX="experiments/reasoner40-active-representation-v1/${RUN_ID}"
STATUS_FILE=/tmp/reasoner40-status.json
TERMINAL_WRITTEN=0
PHASE=bootstrap
...
write_status() {
  ...
  jq -n --arg status "$status_name" --arg phase "$PHASE" \
    ...
    --argjson cost "$cost" \
    '{schema:"zero.reasoner40_aws_status.v1",status:$status,phase:$phase,
...
test "$remaining" -gt 0
set +e
R40_SEALED_EXECUTION=cloud \
R40_SEAL_APPROVAL_ID="$APPROVAL_ID" \
R40_EXECUTION_LOCK=/opt/zero/reasoner40-execution.lock \
  timeout --signal=TERM --kill-after=30s "${remaining}s" \
  ./reasoner40 sealed-run /tmp/result.json > /tmp/sealed-summary.json
runner_exit=$?
set -e
...
jq -e '
  .schema == "zero.reasoner40_active_representation.v1" and
  .version == "4.0" and
  .development_gate_passed == true and
  .planned_sealed_episodes == 6432 and
  .sealed_adapters == 134 and
  .familiar_laws == 6 and
  .sealed.episodes == 6432 and
  .sealed.target_adapters == 134 and
  .sealed.target_laws == 6 and
