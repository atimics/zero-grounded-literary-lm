...
test "$(jq -r .authorization.approval_id benchmarks/zero5-c33-parallel-v1/contract.json)" = "$ZERO5_APPROVAL_ID"
...
request=(ec2 run-instances
  ...
  --user-data file://scripts/aws/zero5-c33-parallel-user-data.sh
  ...
  --tag-specifications "$tags"
    "ResourceType=volume,Tags=[{Key=Project,Value=zero},{Key=Experiment,Value=zero5-c33-parallel-v1},{Key=RunId,Value=${ZERO5_RUN_ID}}]"
...
if [ "$action" = dry-run ]; then
  ...
  echo "ZERO.5 C3.3 parallel AWS dry-run passed"
  exit 0
fi
...
aws s3api put-object --region "$ZERO5_REGION" \
  ...
  --key "experiments/zero5-c33-parallel-v1/${ZERO5_RUN_ID}/launch.json" \
