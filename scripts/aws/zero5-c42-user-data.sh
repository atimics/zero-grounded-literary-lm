...
BOOT_LOG=/var/log/zero5-c42-bootstrap.log
...
RESULT_PREFIX="experiments/zero5-c42-v1/${RUN_ID}"
STATE_PREFIX="${RESULT_PREFIX}/state"
STATUS_FILE=/tmp/zero5-c42-status.json
TERMINAL_WRITTEN=0
PHASE=bootstrap
...
test "$DATASET_DIGEST" = f98728ce8a110e3b949fb0fdb3916b0c9c58d7357172b63003e9c1f3e12c3336
test "$MAX_INSTANCE_SECONDS" = 7941
test "$MAX_COMPUTE_USD" = 1.5
test "$HOURLY_PRICE" = 0.68
test "$APPROVAL_ID" = zero5-c42-aws-2026-08-28-v1
...
write_status() {
  ...
  jq -n --arg status "$status" --arg phase "$PHASE" \
    ...
    --argjson cost "$cost" \
    '{schema:"zero.c42_aws_status.v1",status:$status,phase:$phase,
...
test "$(sha256sum benchmarks/zero5-c42-v1/contract.json | awk '{print $1}')" = "$CONTRACT_SHA256"
...
make zero5_c32_lm_fast zero5_c32_lm_tensor zero5_c32_lm_vector_math
node scripts/check_zero5_c42_aws.mjs
node scripts/run_zero5_c42.mjs --preflight-only >/dev/null

OUT=build/zero5-c42-aws-v1/run
...
write_status running 0
upload_status
runner_args=(--import-dir build/zero5-c42-v1/import-final
...
timeout --signal=TERM --kill-after=90s "${remaining}s" \
  node scripts/run_zero5_c42.mjs "${runner_args[@]}" &
RUNNER_PID=$!
