...
BOOT_LOG=/var/log/reasoner34-bootstrap.log
...
RESULT_PREFIX="experiments/reasoner34-nonmonotonic-planning-v1/${RUN_ID}"
STATUS_FILE=/tmp/reasoner34-status.json
TERMINAL_WRITTEN=0
PHASE=bootstrap
...
write_status() {
  ...
  jq -n --arg status "$status" --arg phase "$PHASE" \
    ...
    --argjson cost "$cost" \
    '{schema:"zero.reasoner34_aws_status.v1",status:$status,phase:$phase,
...
test "$(sha256sum benchmarks/reasoner34-nonmonotonic-planning-v1/aws-contract.json | awk '{print $1}')" = \
  "$CONTRACT_SHA256"
...
R34_SEALED_CLOUD=1 R34_SOURCE_COMMIT="$SOURCE_COMMIT" \
  ...
  ./reasoner34 sealed-run /tmp/result.json > /tmp/sealed-summary.json
runner_exit=$?
set -e
...
jq -e '
  .schema == "zero.reasoner34_nonmonotonic_planning.v1" and
  .version == "(3,3,2)" and
  .sealed_worlds == [5,6,7] and
  .development_gate_passed == true and
  .sealed.semantic.worlds == 5880 and
