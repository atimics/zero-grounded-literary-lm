...
: "${R40_RUN_ID:?R40_RUN_ID is required}"
...
lock_key=experiments/reasoner40-active-representation-v1/execution.lock
...
test "$source_commit" = 73b721a00f8e5737cf0fcfb47b14c90b1e832e70
test "$approval_id" = reasoner40-active-representation-2026-09-01-v1
test "$maximum_seconds" = 900
test "$maximum_ec2_usd" = 0.003
test "$hourly_price" = 0.0104
[[ "$R40_RUN_ID" =~ ^[a-z0-9-]{12,100}$ ]]
[[ "$image_id" =~ ^ami-[0-9a-f]+$ ]]
...
request=(ec2 run-instances
  ...
  --user-data file://scripts/aws/reasoner40-user-data.sh
  ...
  --tag-specifications "$tags"
    "ResourceType=volume,Tags=[{Key=Project,Value=zero},{Key=Experiment,Value=reasoner40-active-representation-v1},{Key=RunId,Value=${R40_RUN_ID}}]"
...
if [ "$action" = dry-run ]; then
  ...
  echo "Reasoner 4.0 AWS dry-run passed"
  exit 0
fi
...
jq -n --arg run_id "$R40_RUN_ID" --arg contract_sha256 "$contract_sha256" \
  ...
  '{schema:"zero.reasoner40_aws_execution_lock.v1",run_id:$run_id,
...
jq -n --arg run_id "$R40_RUN_ID" --arg instance_id "$instance_id" \
  ...
  --argjson maximum_seconds "$maximum_seconds" \
  --argjson maximum_usd "$maximum_ec2_usd" \
  '{schema:"zero.reasoner40_aws_launch.v1",run_id:$run_id,
...
aws s3api put-object --region "$region" --bucket "$bucket" \
  --key "experiments/reasoner40-active-representation-v1/${R40_RUN_ID}/launch.json" \
