...
BOOT_LOG=/var/log/reasoner33-bootstrap.log
...
RESULT_PREFIX="experiments/reasoner33-dimension-transfer-v1/${RUN_ID}"
STATUS_FILE=/tmp/reasoner33-status.json
TERMINAL_WRITTEN=0
PHASE=bootstrap
...
test "$APPROVAL_ID" = reasoner33-cloud-2026-08-30-v1
...
write_status() {
  ...
  jq -n --arg status "$status" --arg phase "$PHASE" \
    ...
    --argjson cost "$cost" \
    '{schema:"zero.reasoner33_aws_status.v1",status:$status,phase:$phase,
...
timeout --signal=TERM --kill-after=30s "${remaining}s" \
  ./reasoner33 sealed-run /tmp/result.json > /tmp/sealed-summary.json
runner_exit=$?
set -e
...
jq -e '
  .schema == "zero.reasoner33_dimension_transfer.v1" and
  .version == "(3,3)" and
  .sealed_dimension == 4 and
  .development_gate_passed == true and
  .semantic.programs == 4095 and
