...
BOOT_LOG=/var/log/weight-multiplicity-phase05-corrective.log
...
PREFIX="experiments/weight-multiplicity-phase05-corrective-cloud-v1/runs/${RUN_ID}"
STATUS=/tmp/weight-multiplicity-corrective-status.json
OUT=/opt/ilxyr/out
PHASE=bootstrap
TERMINAL_WRITTEN=0
SYNC_PID=
...
test "$MAX_INSTANCE_SECONDS" = 2647
test "$WORKLOAD_TIMEOUT_SECONDS" = 2100
test "$MAX_COMPUTE_USD" = 0.50
test "$HOURLY_PRICE" = 0.68
test "$APPROVAL_ID" = weight-multiplicity-phase05-corrective-cloud-2026-08-30-v1
[[ "$RUN_ID" =~ ^[0-9]+$ ]]
...
write_status() {
  ...
  jq -n --arg status "$status" --arg phase "$PHASE" \
    ...
    --argjson cost "$cost" \
    '{schema:"ilxyr.weight_multiplicity_corrective_cloud_status.v1",
...
test "$(jq -r .evidence_status lie-cross-check-v5.json)" = pass
test "$(jq -r '.changes | length' exactness-correction-v1.json)" = 2

cp lie-cross-check-v5.json exactness-correction-v1.json execution-record.json "$OUT/"
( while true; do sync_outputs || true; sleep 30; done ) &
SYNC_PID=$!

PHASE=allocator_audit
write_status running 0
upload_status
remaining=$((LAUNCH_EPOCH + WORKLOAD_TIMEOUT_SECONDS - $(date +%s)))
test "$remaining" -gt 0
timeout --signal=TERM --kill-after=60s "${remaining}s" \
  node scripts/run-weight-multiplicity-phase05-allocator-audit.mjs \
    --audit-plan examples/weight-multiplicity/phase05-cloud-corrective-audit-v1.json \
    --source-plan examples/weight-multiplicity/phase05-cloud-frontier-plan.json \
    --oracle ./weight_multiplicity \
    --out "$OUT/allocator-audit-v1.json"

test "$(jq -r .evidence_status "$OUT/allocator-audit-v1.json")" = complete
cd "$OUT"
sha256sum allocator-audit-v1.json exactness-correction-v1.json \
  lie-cross-check-v5.json execution-record.json > sha256sums.txt
