#!/bin/bash
# EC2 bootstrap for the bounded parallel quantity-evaluator calibration.

set -Eeuo pipefail

HARD_INSTANCE_SECONDS=1500
HARD_WORKLOAD_SECONDS=1440
BOOTSTRAP_LOG=/var/log/zero-parallel-quantity-eval-calibration-bootstrap.log
BOOTSTRAP_STARTED_AT=$(date -u +%Y-%m-%dT%H:%M:%SZ)

exec > >(tee -a "$BOOTSTRAP_LOG" >/dev/console) 2>&1
set -x

# GitHub enforces the same launch-relative cap. This local watchdog protects
# the budget if the control-plane observer fails.
(
  sleep "$HARD_INSTANCE_SECONDS"
  echo "Parallel quantity-evaluator calibration local deadline reached"
  shutdown -h now
) &
...
finish() {
  ...
  if command -v aws >/dev/null 2>&1 \
    ...
    aws s3 cp "$BOOTSTRAP_LOG" \
      "s3://${ZERO_BUCKET}/jobs/${ZERO_RUN_ID}/zero-parallel-quantity-eval-calibration-bootstrap.log" \
      --no-cli-pager
    if [ "$exit_code" -ne 0 ] \
...
with open("/tmp/zero-parallel-quantity-eval-status.json", "w", encoding="utf-8") as handle:
    json.dump(record, handle, indent=2)
    handle.write("\n")
PY
      aws s3 cp /tmp/zero-parallel-quantity-eval-status.json \
...
apt-get install -y -qq \
  build-essential ca-certificates curl jq nodejs npm python3 unzip
...
export ZERO_HOURLY_RATE_USD AWS_DEFAULT_REGION
...
aws s3 cp \
  "s3://${ZERO_BUCKET}/jobs/${ZERO_RUN_ID}/parallel-quantity-eval-calibration.sh" \
  /opt/zero/parallel-quantity-eval-calibration.sh \
  --no-cli-pager
chmod 0755 /opt/zero/parallel-quantity-eval-calibration.sh
...
timeout --signal=TERM --kill-after=10s \
  "${remaining}s" /opt/zero/parallel-quantity-eval-calibration.sh
