...
BOOTSTRAP_LOG=/var/log/zero-q28-u100-language-gate-bootstrap.log
...
test "$ZERO_BUDGET_FILE" = \
  benchmarks/zero4-q28-v1/update-100-language-gate/results/runtime-budget.json
...
aws s3 cp "s3://${ZERO_BUCKET}/jobs/${ZERO_RUN_ID}/workload.sh" \
  /opt/zero/q28-u100-language-gate.sh --no-cli-pager
test "$(sha256sum /opt/zero/q28-u100-language-gate.sh | awk '{print $1}')" = \
  "$ZERO_WORKLOAD_SHA256"
chmod 0755 /opt/zero/q28-u100-language-gate.sh
...
timeout --signal=TERM --kill-after=10s "${remaining}s" \
  /opt/zero/q28-u100-language-gate.sh
