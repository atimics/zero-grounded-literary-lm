...
request=(ec2 run-instances
  ...
  --tag-specifications "$tags"
    "ResourceType=volume,Tags=[{Key=Project,Value=sero},{Key=Experiment,Value=sero2-curriculum-v1},{Key=RunId,Value=${SERO_RUN_ID}}]"
...
if [ "$action" = dry-run ]; then
  ...
  echo "Sero curriculum ${mode} seed 0 dry-run passed"
else
