...
BOOT_LOG=/var/log/reasoner41-bootstrap.log
...
RESULT_PREFIX="experiments/reasoner41-joint-transfer-v1/${RUN_ID}"
STATUS_FILE=/tmp/reasoner41-status.json
TERMINAL_WRITTEN=0
PHASE=bootstrap
...
write_status() {
  ...
  jq -n --arg status "$status_name" --arg phase "$PHASE" \
    ...
    --argjson cost "$cost" \
    '{schema:"zero.reasoner41_aws_status.v1",status:$status,phase:$phase,
...
R41_SEAL_APPROVAL_ID="$APPROVAL_ID" \
R41_EXECUTION_LOCK=/opt/zero/reasoner41-execution.lock \
  timeout --signal=TERM --kill-after=30s "${remaining}s" \
  ./reasoner41 sealed-run /tmp/result.json > /tmp/sealed-summary.json
runner_exit=$?
set -e
...
jq -e '
  .schema == "zero.reasoner41_joint_transfer.v1" and
  .version == "4.1" and
  .development_gate_passed == true and
  .planned_sealed_episodes == 33232 and
  .planned_sealed_pairs == 4154 and
  .sealed_adapters == 134 and
  .sealed_laws == 31 and
  .sealed.episodes == 33232 and
  .sealed.target_adapters == 134 and
  .sealed.target_laws == 31 and
  .sealed.target_pairs == 4154 and
