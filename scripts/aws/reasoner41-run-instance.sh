...
: "${R41_RUN_ID:?R41_RUN_ID is required}"
contract=benchmarks/reasoner41-joint-transfer-v1/contract.json
...
lock_key=experiments/reasoner41-joint-transfer-v1/execution.lock
...
test "$source_commit" = 8153537581137822a1fa5e3c98139c60238ba8d4
test "$approval_id" = reasoner41-joint-transfer-2026-09-01-v1
test "$maximum_seconds" = 2400
test "$maximum_ec2_usd" = 0.007
test "$hourly_price" = 0.0104
[[ "$R41_RUN_ID" =~ ^[a-z0-9-]{12,100}$ ]]
[[ "$image_id" =~ ^ami-[0-9a-f]+$ ]]
...
request=(ec2 run-instances
  ...
  --user-data file://scripts/aws/reasoner41-user-data.sh
  ...
  --tag-specifications "$tags"
    "ResourceType=volume,Tags=[{Key=Project,Value=zero},{Key=Experiment,Value=reasoner41-joint-transfer-v1},{Key=RunId,Value=${R41_RUN_ID}}]"
...
if [ "$action" = dry-run ]; then
  ...
  echo "Reasoner 4.1 AWS dry-run passed"
  exit 0
fi
...
jq -n --arg run_id "$R41_RUN_ID" --arg contract_sha256 "$contract_sha256" \
  ...
  '{schema:"zero.reasoner41_aws_execution_lock.v1",run_id:$run_id,
...
jq -n --arg run_id "$R41_RUN_ID" --arg instance_id "$instance_id" \
  ...
  --argjson maximum_seconds "$maximum_seconds" \
  --argjson maximum_usd "$maximum_ec2_usd" \
  '{schema:"zero.reasoner41_aws_launch.v1",run_id:$run_id,
...
aws s3api put-object --region "$region" --bucket "$bucket" \
  --key "experiments/reasoner41-joint-transfer-v1/${R41_RUN_ID}/launch.json" \
