...
RESULTS_ROOT=/tmp/zero-results/zero4-q22r-v1
...
for seed in "${seed_array[@]}"; do
  case "$seed" in
    1|3) ;;
    *)
      echo "Q2.2-R replication accepts only outstanding seeds 1 and 3; got $seed" >&2
      exit 1
      ;;
  esac

  echo "=== Q2.2-R acquisition replication seed $seed ==="
  seed_dir="$RESULTS_ROOT/seed${seed}"
  prefix="/tmp/zero4-q22r-seed${seed}"
  mkdir -p "$seed_dir"
...
echo "Training complete; seed-level results uploaded."
