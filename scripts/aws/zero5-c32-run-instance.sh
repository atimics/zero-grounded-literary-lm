...
for name in ZERO5_AMI ZERO5_SECURITY_GROUP_ID ZERO5_SUBNET_ID \
  ...
  ZERO5_CONTRACT_SHA256 ZERO5_CONTINUATION_SHA256 ZERO5_APPROVAL_ID; do
...
[[ "$ZERO5_CONTINUATION_SHA256" =~ ^[0-9a-f]{64}$ ]]
...
if [ "$action" = resume ]; then
  ...
  test "$(jq -r .approval_id "$lock_file")" = zero5-c32-aws-2026-08-24-v1
  prior_status=$(mktemp)
  aws s3 cp \
...
if [ "$action" = dry-run ]; then
  ...
  jq -n --arg run_id "$ZERO5_RUN_ID" --arg instance_id "$instance_id" \
    ...
    --arg continuation_sha256 "$ZERO5_CONTINUATION_SHA256" \
    ...
    '{schema:"zero.c32_aws_launch.v1",run_id:$run_id,instance_id:$instance_id,
      ...
      continuation_sha256:$continuation_sha256,approval_id:$approval_id,
