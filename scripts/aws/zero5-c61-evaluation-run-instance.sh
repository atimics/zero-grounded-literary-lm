...
for name in ZERO5_AMI ZERO5_SECURITY_GROUP_ID ZERO5_SUBNET_ID \
  ...
  ZERO5_RECOVERY_CONTRACT_SHA256 ZERO5_APPROVAL_ID; do
...
lock_key=experiments/zero5-c61-shared-state-v1/evaluation-v1.lock

test "$ZERO5_REGION" = us-east-1
test "$ZERO5_APPROVAL_ID" = \
  zero5-c61-evaluation-recovery-aws-2026-09-01-v1
test "$(jq -r .authorized benchmarks/zero5-c61-shared-state-v1/evaluation-recovery-contract.json)" = true
test "$(jq -r .training_authorized benchmarks/zero5-c61-shared-state-v1/evaluation-recovery-contract.json)" = false
test "$(jq -r .authorization.approval_id benchmarks/zero5-c61-shared-state-v1/evaluation-recovery-contract.json)" = "$ZERO5_APPROVAL_ID"
test "$(jq -r .execution.venue benchmarks/zero5-c61-shared-state-v1/evaluation-recovery-contract.json)" = "aws us-east-1 c6i.4xlarge on-demand"
...
jq -n --arg run_id "$ZERO5_RUN_ID" \
  ...
  '{schema:"zero.c61_evaluation_aws_execution_lock.v1",run_id:$run_id,
...
jq -n --arg run_id "$ZERO5_RUN_ID" --arg instance_id "$instance_id" \
  ...
  --argjson maximum_seconds "$maximum_seconds" \
  --argjson maximum_usd "$max_usd" \
  '{schema:"zero.c61_evaluation_aws_launch.v1",run_id:$run_id,
...
aws s3api put-object --region "$ZERO5_REGION" \
  ...
  --key "experiments/zero5-c61-shared-state-v1/evaluation-recovery/${ZERO5_RUN_ID}/launch.json" \
