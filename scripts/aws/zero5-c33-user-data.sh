...
BOOT_LOG=/var/log/zero5-c33-bootstrap.log
...
RESULT_PREFIX="experiments/zero5-c33-v1/${RUN_ID}"
STATE_PREFIX="${RESULT_PREFIX}/state"
STATUS_FILE=/tmp/zero5-c33-status.json
TERMINAL_WRITTEN=0
PHASE=bootstrap
...
test "$MAX_COMPUTE_USD" = 3.4
test "$HOURLY_PRICE" = 0.68
test "$APPROVAL_ID" = zero5-c33-aws-2026-08-26-v1
awk -v prior="$PRIOR_COMPUTE_USD" 'BEGIN { exit !(prior >= 0 && prior < 3.4) }'
...
write_status() {
  ...
  jq -n --arg status "$status" --arg phase "$PHASE" \
    ...
    --argjson cost "$cost" \
    '{schema:"zero.c33_aws_status.v1",status:$status,phase:$phase,
...
test "$(sha256sum benchmarks/zero5-c33-v1/contract.json | awk '{print $1}')" = \
  "$CONTRACT_SHA256"

PHASE=assets
write_status running 0
upload_status
aws s3 cp "s3://${TRAINING_BUCKET}/${ASSET_KEY}" /tmp/zero5-c33-assets.tar.gz \
  --only-show-errors
test "$(sha256sum /tmp/zero5-c33-assets.tar.gz | awk '{print $1}')" = \
  "$ASSET_SHA256"
tar -xzf /tmp/zero5-c33-assets.tar.gz -C /opt/zero/repo
...
OUT=build/zero5-c33-aws-v1/run
...
if [ "$START_EVENT_EXISTS" -eq 0 ]; then
  ...
  jq -n '{status:"running",experiment:"zero5-c33-v1",seed:0,arm:"E",
    ...
    > /tmp/zero5-c33-event.json
  publish_event "$((last_update + 2))" phase.started /tmp/zero5-c33-event.json
fi
...
publish_latest_metric() {
  log="$OUT/E/training.log"
  marker="$OUT/telemetry-E-update"
  test -f "$log" || return 0
  ...
  jq -n --argjson update "$update" --argjson loss "$loss" \
    '{status:"running",experiment:"zero5-c33-v1",seed:0,arm:"E",
      update:$update,loss:$loss,note:"Pair-atomic C3.3 training metric."}' \
    > /tmp/zero5-c33-metric.json
  publish_event "$update" metric /tmp/zero5-c33-metric.json
  printf '%s\n' "$update" > "$marker"
}
monitor() {
  while kill -0 "$RUNNER_PID" 2>/dev/null; do
    sync_state || true
    publish_latest_metric || true
    sleep 30
  done
}
...
write_status running 0
upload_status
runner_args=(--import-dir build/zero5-c33-v1/import-final
...
if [ "$runner_exit" -eq 0 ] && [ -s "$OUT/result.json" ]; then
  ...
  cost=$(estimated_cost "$(elapsed_seconds)")
  decision=$(jq -r .decision.outcome "$OUT/result.json")
  mean_choice=$(jq -r .arms.E.validation.mean_paired_choice_accuracy "$OUT/result.json")
  mean_swap=$(jq -r .arms.E.validation.mean_swap_consistency_accuracy "$OUT/result.json")
  mean_exact=$(jq -r .arms.E.validation.mean_pair_exact_accuracy "$OUT/result.json")
  jq -n --arg decision "$decision" --argjson compute_usd "$cost" \
    --argjson choice "$mean_choice" --argjson swap "$mean_swap" \
    --argjson exact "$mean_exact" \
    '{status:"completed",experiment:"zero5-c33-v1",seed:0,arm:"E",
      update:9442,decision:$decision,compute_usd:$compute_usd,
      accuracy:$choice,metric_kind:"paired-invariance",
      mean_choice_accuracy:$choice,mean_swap_consistency_accuracy:$swap,
      mean_pair_exact_accuracy:$exact,test_metrics_opened:false,
      note:"Pair-atomic C3.3 screen completed; test remained sealed."}' \
    > /tmp/zero5-c33-complete.json
  publish_event 20000 run.completed /tmp/zero5-c33-complete.json
  PHASE=complete
...
upload_status
jq -n '{status:"failed",experiment:"zero5-c33-v1",seed:0,arm:"E",
  note:"C3.3 stopped before completion; contract-bound checkpoint state is available."}' \
  > /tmp/zero5-c33-recoverable.json
last_update=0
test ! -f "$OUT/telemetry-E-update" || last_update=$(cat "$OUT/telemetry-E-update")
publish_event "$((last_update + 1))" run.failed /tmp/zero5-c33-recoverable.json
TERMINAL_WRITTEN=1
exit "$runner_exit"
