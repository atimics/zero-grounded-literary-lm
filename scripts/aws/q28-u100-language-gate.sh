...
RESULTS_ROOT=/tmp/zero-q28-u100-language-gate-results
RESULT_FILE="$RESULTS_ROOT/result.json"
STATUS_FILE=/tmp/zero-q28-u100-language-gate-status.json
WORKLOAD_LOG=/var/log/zero-q28-u100-language-gate.log
...
write_failure_status() {
  ...
  jq -n \
    ...
    '{
      schema: "zero.q28_u100_language_gate_status.v1",
      status: $status,
      phase: $phase,
      exit_code: $exit_code,
...
heartbeat() {
  jq -n --arg phase "$PHASE" --arg git_commit "$ZERO_COMMIT" \
    ...
    '{
      schema: "zero.q28_u100_language_gate_heartbeat.v1",
      phase: $phase,
      git_commit: $git_commit,
      candidate_sha256: $candidate_sha256,
      at: $at,
      elapsed_instance_seconds: $elapsed
    }' > /tmp/zero-q28-u100-language-gate-heartbeat.json
  aws s3 cp /tmp/zero-q28-u100-language-gate-heartbeat.json \
...
PHASE=source
heartbeat
install -d -m 0755 /tmp/zero /tmp/zero-q28-screen
aws s3 cp "s3://${ZERO_BUCKET}/jobs/${ZERO_RUN_ID}/source.tar.gz" \
  /tmp/zero-q28-source.tar.gz --no-cli-pager
aws s3 cp "s3://${ZERO_BUCKET}/jobs/${ZERO_RUN_ID}/screen.tar.gz" \
  /tmp/zero-q28-screen.tar.gz --no-cli-pager
test "$(sha256sum /tmp/zero-q28-source.tar.gz | awk '{print $1}')" = \
  "$ZERO_SOURCE_SHA256"
test "$(sha256sum /tmp/zero-q28-screen.tar.gz | awk '{print $1}')" = \
  "$ZERO_SCREEN_SHA256"
tar -xzf /tmp/zero-q28-source.tar.gz -C /tmp/zero
tar -xzf /tmp/zero-q28-screen.tar.gz -C /tmp/zero-q28-screen
cd /tmp/zero
...
timeout --signal=TERM --kill-after=10s "${remaining}s" \
  node scripts/run_zero_language_gate.mjs \
    ...
    --model benchmarks/zero4-q28-v1/update-100-language-gate/candidate.litq8 \
    --model-id zero4-q28-seed2-u100 \
    --blimp /tmp/zero-q28-screen/blimp.tsv \
    --tinystories /tmp/zero-q28-screen/tinystories.tsv \
