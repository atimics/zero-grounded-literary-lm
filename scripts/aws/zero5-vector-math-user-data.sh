...
BOOT_LOG=/var/log/zero5-vector-math-bootstrap.log
...
RESULT_PREFIX="experiments/zero5-vector-math-v1/${RUN_ID}"
STATUS_FILE=/tmp/zero5-vector-math-status.json
...
write_status() {
  ...
  jq -n --arg status "$status" --arg phase "$PHASE" \
    ...
    --argjson cost "$cost" \
    '{schema:"zero.aws_vector_math_status.v1",status:$status,phase:$phase,
...
make zero5_c32_lm_fast zero5_c32_lm_vector_tanh \
...
test "$remaining" -gt 0
calibration_timeout=250
if [ "$remaining" -lt "$calibration_timeout" ]; then
  calibration_timeout=$remaining
fi
timeout --signal=TERM --kill-after=15s "${calibration_timeout}s" \
  node scripts/benchmark_zero5_vector_math.mjs \
    --asset-root /opt/zero/repo --updates 50 --repetitions 2 --skip-build \
    --out "$OUT/result.json" > "$OUT/calibration.log" 2>&1
test -s "$OUT/result.json"
jq -e '
  .schema == "zero.vector_math_benchmark.v1" and
  .workload.updates == 50 and
  .workload.repetitions == 2 and
  .workload.parallel_workers == 4 and
  .correctness.all_metrics_within_tolerance == true and
  .correctness.deterministic_checkpoints_per_variant == true and
  ((.variants | keys | sort) ==
    ["baseline","vector-both","vector-exp","vector-tanh"])
' "$OUT/result.json" >/dev/null
upload_file "$OUT/calibration.log" calibration.log
