...
BOOT_LOG=/var/log/sero1-optimized-bootstrap.log
...
RESULT_PREFIX="experiments/sero1-optimized-v1/${RUN_ID}"
STATUS_FILE=/tmp/sero1-optimized-status.json

export AWS_DEFAULT_REGION SERO_SOURCE_COMMIT="$SOURCE_COMMIT"
test "$AWS_DEFAULT_REGION" = us-east-1
test "$INSTANCE_TYPE" = g5.xlarge
test "$DATASET_DIGEST" = 0cefd7a464177abec1ce32349aca957ac2a82d4d53cb0c9a7775defb13ace82d
...
write_status() {
  ...
  jq -n --arg status "$status" --arg phase "$PHASE" --arg mode "$MODE" \
    ...
    --arg branch_key "$branch_key" --arg branch_sha256 "$branch_sha256" \
    --argjson seed "$SEED" --argjson exit_code "$exit_code" \
    --argjson elapsed "$elapsed" --argjson cost "$cost" \
    '{schema:"sero.optimized_pretrain_aws_status.v1",status:$status,phase:$phase,
      ...
      branch_key:(if $branch_key=="" then null else $branch_key end),
      branch_artifact_sha256:(if $branch_sha256=="" then null else $branch_sha256 end)}' \
    > "$STATUS_FILE"
}
upload_status() {
...
write_status running 0
upload_status
install -d -m 0755 build/sero-pretrain-v2
aws s3 sync "s3://${CORPUS_BUCKET}/${DATASET_PREFIX}" build/sero-pretrain-v2 \
  --only-show-errors
python3 scripts/verify_zero_dataset.py --root build/sero-pretrain-v2 \
  --digest "$DATASET_DIGEST"

PHASE=environment
write_status running 0
upload_status
python3 -m venv build/sero1-optimized/aws-venv
build/sero1-optimized/aws-venv/bin/pip install --disable-pip-version-check \
  --no-input -r experiments/sero1-pretrain/requirements.txt
nvidia-smi > build/sero1-optimized/nvidia-smi.txt
build/sero1-optimized/aws-venv/bin/python - <<'PY'
import torch
if not torch.cuda.is_available():
    raise RuntimeError("CUDA is unavailable")
print(torch.cuda.get_device_name(0))
PY
build/sero1-optimized/aws-venv/bin/pip freeze > build/sero1-optimized/aws-pip-freeze.txt
build/sero1-optimized/aws-venv/bin/python experiments/sero1-optimized/tests.py \
  --manifest build/sero-pretrain-v2/manifest.json

PHASE=training
write_status running 0
upload_status
RESULT_FILE="build/sero1-optimized/${MODE}/seed0.json"
ARTIFACT_DIR="build/sero1-optimized/${MODE}/artifacts"
TRAIN_ARGS=(--mode "$MODE")
if [ "$MODE" = calibration ]; then
  TRAIN_ARGS+=(--max-updates 128 --validation-byte-limit 131072)
elif [ "$MODE" = unlikelihood ]; then
  aws s3 cp "s3://${TRAINING_BUCKET}/${RESUME_KEY}" /tmp/model-branch-epoch5.pt \
    --only-show-errors
  test "$(sha256sum /tmp/model-branch-epoch5.pt | awk '{print $1}')" = "$RESUME_SHA256"
  TRAIN_ARGS+=(--resume /tmp/model-branch-epoch5.pt)
fi
...
timeout --signal=TERM --kill-after=30s "${remaining}s" \
  build/sero1-optimized/aws-venv/bin/python experiments/sero1-optimized/train.py \
    --contract benchmarks/sero1-optimized-v1/contract.json \
    --manifest build/sero-pretrain-v2/manifest.json \
...
BRANCH_KEY=
BRANCH_SHA256=
if [ -f "$ARTIFACT_DIR/model-branch-epoch5.pt" ]; then
  BRANCH_KEY="${RESULT_PREFIX}/artifacts/model-branch-epoch5.pt"
  BRANCH_SHA256=$(sha256sum "$ARTIFACT_DIR/model-branch-epoch5.pt" | awk '{print $1}')
fi
...
aws s3 cp build/sero1-optimized/nvidia-smi.txt \
  "s3://${TRAINING_BUCKET}/${RESULT_PREFIX}/nvidia-smi.txt" --only-show-errors
aws s3 cp build/sero1-optimized/aws-pip-freeze.txt \
  "s3://${TRAINING_BUCKET}/${RESULT_PREFIX}/pip-freeze.txt" --only-show-errors
PHASE=complete
write_status complete 0 "$RESULT_KEY" "$RESULT_SHA256" "$MODEL_KEY" \
  "$MODEL_SHA256" "$BRANCH_KEY" "$BRANCH_SHA256"
