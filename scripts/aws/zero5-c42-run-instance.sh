...
max_usd=1.5
hourly_price=0.68
maximum_seconds=7941
lock_key=experiments/zero5-c42-v1/execution-v1.lock

test "$ZERO5_REGION" = us-east-1
test "$ZERO5_APPROVAL_ID" = zero5-c42-aws-2026-08-28-v1
test "$(jq -r .authorized benchmarks/zero5-c42-v1/contract.json)" = true
test "$(jq -r .authorization.approval_id benchmarks/zero5-c42-v1/contract.json)" = "$ZERO5_APPROVAL_ID"
test "$(jq -r .execution.maximum_instance_seconds benchmarks/zero5-c42-v1/contract.json)" = "$maximum_seconds"
test "$(jq -r .execution.maximum_total_ec2_usd benchmarks/zero5-c42-v1/contract.json)" = "$max_usd"
...
launch_epoch=$(date +%s)
tags="ResourceType=instance,Tags=[{Key=Project,Value=zero},{Key=Name,Value=zero5-c42},{Key=Experiment,Value=zero5-c42-v1},{Key=Commit,Value=${ZERO5_SOURCE_COMMIT}},{Key=RunId,Value=${ZERO5_RUN_ID}},{Key=SourceKey,Value=${ZERO5_SOURCE_KEY}},{Key=SourceSha256,Value=${ZERO5_SOURCE_SHA256}},{Key=AssetKey,Value=${ZERO5_ASSET_KEY}},{Key=AssetSha256,Value=${ZERO5_ASSET_SHA256}},{Key=TrainingBucket,Value=${ZERO5_TRAINING_BUCKET}},{Key=DatasetDigest,Value=f98728ce8a110e3b949fb0fdb3916b0c9c58d7357172b63003e9c1f3e12c3336},{Key=ContractSha256,Value=${ZERO5_CONTRACT_SHA256}},{Key=Region,Value=${ZERO5_REGION}},{Key=LaunchEpoch,Value=${launch_epoch}},{Key=MaxInstanceSeconds,Value=${maximum_seconds}},{Key=MaxComputeUsd,Value=${max_usd}},{Key=HourlyPrice,Value=${hourly_price}},{Key=ApprovalId,Value=${ZERO5_APPROVAL_ID}}]"
...
request=(ec2 run-instances
  ...
  --user-data file://scripts/aws/zero5-c42-user-data.sh
  ...
  --tag-specifications "$tags"
    "ResourceType=volume,Tags=[{Key=Project,Value=zero},{Key=Experiment,Value=zero5-c42-v1},{Key=RunId,Value=${ZERO5_RUN_ID}}]"
...
if [ "$action" = dry-run ]; then
  ...
  echo "ZERO.5 C4.2 AWS dry-run passed"
  exit 0
fi
...
jq -n --arg run_id "$ZERO5_RUN_ID" \
  ...
  '{schema:"zero.c42_aws_execution_lock.v1",run_id:$run_id,
...
jq -n --arg run_id "$ZERO5_RUN_ID" --arg instance_id "$instance_id" \
  ...
  --argjson maximum_seconds "$maximum_seconds" \
  --argjson maximum_usd "$max_usd" \
  '{schema:"zero.c42_aws_launch.v1",run_id:$run_id,
...
aws s3api put-object --region "$ZERO5_REGION" \
  ...
  --key "experiments/zero5-c42-v1/${ZERO5_RUN_ID}/launch.json" \
