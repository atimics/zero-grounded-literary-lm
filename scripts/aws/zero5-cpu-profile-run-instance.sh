...
contract=benchmarks/zero5-cpu-phase-profile-v1/aws-contract.json
...
request=(ec2 run-instances
  ...
  --tag-specifications "$tags"
    "ResourceType=volume,Tags=[{Key=Project,Value=zero},{Key=Experiment,Value=zero5-cpu-phase-profile-v1},{Key=RunId,Value=${ZERO5_RUN_ID}}]"
...
if [ "$action" = dry-run ]; then
  ...
  echo "ZERO.5 CPU profile AWS dry-run passed"
  exit 0
fi

lock_key=experiments/zero5-cpu-phase-profile-v1/execution.lock
lock=$(mktemp)
jq -n --arg run_id "$ZERO5_RUN_ID" \
  ...
  '{schema:"zero.aws_cpu_phase_profile_execution_lock.v1",run_id:$run_id,
...
jq -n --arg run_id "$ZERO5_RUN_ID" --arg instance_id "$instance_id" \
  ...
  --argjson maximum_usd "$max_usd" \
  '{schema:"zero.aws_cpu_phase_profile_launch.v1",run_id:$run_id,
...
aws s3api put-object --region "$region" \
  ...
  --key "experiments/zero5-cpu-phase-profile-v1/${ZERO5_RUN_ID}/launch.json" \
