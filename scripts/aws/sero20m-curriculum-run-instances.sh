...
case "$mode" in
  calibration) max_seconds=3600; max_usd=1.006 ;;
  full) max_seconds=21600; max_usd=6.036 ;;
esac
...
request=(ec2 run-instances
  ...
  --tag-specifications "$tags"
    "ResourceType=volume,Tags=[{Key=Project,Value=sero},{Key=Experiment,Value=sero20m-curriculum-v1},{Key=RunId,Value=${SERO_RUN_ID}}]"
...
if [ "$action" = dry-run ]; then
  ...
  echo "Sero 20M curriculum ${mode} seed 0 dry-run passed"
else
