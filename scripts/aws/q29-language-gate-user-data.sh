...
BOOTSTRAP_LOG=/var/log/zero-q29-language-gate-bootstrap.log
...
test "$ZERO_BUDGET_FILE" = \
  benchmarks/zero4-q29-v1/language-gate/results/runtime-budget.json
...
aws s3 cp "s3://${ZERO_BUCKET}/jobs/${ZERO_RUN_ID}/workload.sh" \
  /opt/zero/q29-language-gate.sh --no-cli-pager
test "$(sha256sum /opt/zero/q29-language-gate.sh | awk '{print $1}')" = \
  "$ZERO_WORKLOAD_SHA256"
chmod 0755 /opt/zero/q29-language-gate.sh
...
if [ "$remaining" -le 0 ]; then
  jq -n --arg instance_id "$ZERO_INSTANCE_ID" \
    ...
    }' > /tmp/q29-cold-start-status.json
  aws s3 cp /tmp/q29-cold-start-status.json \
