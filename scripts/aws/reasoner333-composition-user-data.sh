...
BOOT_LOG=/var/log/reasoner333-composition-bootstrap.log
...
RESULT_PREFIX="experiments/reasoner333-composition-transfer-v1/${RUN_ID}"
STATUS_FILE=/tmp/reasoner333-composition-status.json
TERMINAL_WRITTEN=0
PHASE=bootstrap
...
test "$APPROVAL_ID" = reasoner333-composition-seal-2026-08-30-v1
test "$SOURCE_COMMIT" = f7465586b0133b82c6627162b75df5a28d25883b
...
write_status() {
  ...
  jq -n --arg status "$status_name" --arg phase "$PHASE" \
    ...
    --argjson cost "$cost" \
    '{schema:"zero.reasoner333_composition_aws_status.v1",status:$status,
...
R333_SEAL_APPROVAL_ID="$APPROVAL_ID" \
  ...
  ./reasoner333 sealed-run /tmp/result.json > /tmp/sealed-summary.json
runner_exit=$?
set -e
...
jq -e '
  .schema == "zero.reasoner333_composition_transfer.v1" and
  .version == "(3,3,3)" and
  .development_gate_passed == true and
  .semantic.programs == 63 and
  .semantic.relabel_cases == 252 and
