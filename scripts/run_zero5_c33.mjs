...
if (contract.status !== "preregistered-unrun" || !contract.authorized) {
  fail("C3.3 contract is not authorized");
}
if (sha256(fs.readFileSync(contract.implementation.trainer)) !==
  ...
  fail("C3.3 implementation drifted from the contract");
}
...
if (sha256(fs.readFileSync(contract.input.import_manifest)) !==
  ...
  fail("C3.3 import manifest drifted");
}
if (imported.schema !== "zero.c33_import.v1" ||
    ...
    imported.paired_training.optimizer_batches !==
      contract.training.updates) {
  fail("C3.3 paired import contract is invalid");
}
...
const out = path.resolve(option("--out", "build/zero5-c33-v1/run"));
...
if (fs.existsSync(out) && fs.readdirSync(out).length > 0) {
  ...
  if (execution.schema !== "zero.c33_execution.v1" ||
      execution.contract_sha256 !== contractSha256) {
    fail("resume output belongs to a different C3.3 contract");
  }
} else {
  fs.mkdirSync(out, { recursive: true });
  fs.writeFileSync(executionPath, JSON.stringify({
    schema: "zero.c33_execution.v1",
...
const controlResultBytes = fs.readFileSync(contract.control_source.result);
if (sha256(controlResultBytes) !== contract.control_source.result_sha256) {
  fail("the frozen C3.2 control result changed");
}
const controlResult = JSON.parse(controlResultBytes);
if (controlResult.schema !== "zero.c32_braid_result.v1" ||
    controlResult.status !== "complete" ||
    controlResult.test.metrics_opened !== false) {
  fail("the frozen C3.2 control result is invalid");
}
const baseline = controlResult.baselines;
process.stdout.write("C3.3 using the hash-bound C3.2/C2 baseline\n");
const observedControl = controlResult.arms.D.validation;
for (const [name, observed] of [
  ["combined_final_nats_per_token",
    observedControl.combined_final_nats_per_token],
  ["mean_paired_choice_accuracy",
    observedControl.mean_paired_choice_accuracy],
  ["mean_pair_exact_accuracy", observedControl.mean_pair_exact_accuracy],
]) {
  if (!close(observed, contract.control_c32_D[name], 1e-9)) {
    fail("the frozen C3.2 D control changed: " + name);
  }
}
if (!close(baseline.combined_selection_nats_per_token,
    ...
    fail("C3.3 whole-distribution baseline changed");
}
for (const task of Object.keys(files.tasks)) {
  if (!close(baseline.members[task],
    ...
    fail(task + " C3.3 answer baseline changed");
  }
}
for (const task of ["claim", "retrieval"]) {
  for (const metric of ["forced_choice_nats", "choice_accuracy",
    ...
    if (!close(baseline.paired[task][metric],
      ...
      fail(task + " C3.3 paired baseline changed: " + metric);
    }
  }
}
...
if (preflightOnly) {
  process.stdout.write(JSON.stringify({
    schema: "zero.c33_preflight.v1",
    contract_sha256: contractSha256,
    train_packs_sha256: artifact(files.interleaved).sha256,
    pair_cross_batch_leakage:
      imported.paired_training.pair_cross_batch_leakage,
    updates: contract.training.updates,
    compute_token_exposures: contract.training.compute_token_exposures,
    control_result_sha256: sha256(controlResultBytes),
...
for (const definition of armDefinitions) {
  ...
  if (fs.existsSync(armResultPath)) {
    ...
    process.stdout.write("C3.3 arm " + definition.id +
  ...
  process.stdout.write("C3.3 arm " + definition.id + " starting\n");
  ...
  validation.mean_swap_consistency_accuracy =
    (validation.paired.claim.swap_consistency_accuracy +
      validation.paired.retrieval.swap_consistency_accuracy) / 2;
  const gates = {
    ...
    retrieval_choice_retention:
      validation.paired.retrieval.choice_accuracy >=
  ...
  arms[definition.id] = {
    definition: {
      pack_order: "pair-atomic-task-smoothed",
  ...
  process.stdout.write("C3.3 arm " + definition.id + " complete gates=" +
...
const control = contract.control_c32_D;
const result = {
  schema: "zero.c33_pair_atomic_result.v1",
  experiment: contract.experiment,
  status: "complete",
  ...
  baselines: baseline,
  control_c32_D: control,
  arms,
  comparisons: {
    pair_atomic_E_minus_D_mean_swap_consistency_accuracy:
      arms.E.validation.mean_swap_consistency_accuracy -
        control.mean_swap_consistency_accuracy,
    pair_atomic_E_minus_D_mean_choice_accuracy:
      arms.E.validation.mean_paired_choice_accuracy -
        control.mean_paired_choice_accuracy,
    pair_atomic_E_minus_D_mean_pair_exact_accuracy:
      arms.E.validation.mean_pair_exact_accuracy -
        control.mean_pair_exact_accuracy,
    pair_atomic_screen_supported: arms.E.all_gates_pass,
  },
  ...
  decision: {
    ...
    outcome: preferredArm === null ? "no-go" : "pass-c33-screen",
    single_seed: true,
