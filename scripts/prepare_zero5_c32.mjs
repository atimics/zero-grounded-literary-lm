...
const out = path.resolve(option("--out", "build/zero5-c32-v1/import"));
if (!expectedReleaseId || !expectedManifestSha256) {
  fail("--release-id and --manifest-sha256 are required");
}
...
const result = {
  schema: "zero.c32_import.v1",
  release: {
    id: expectedReleaseId,
    braid_head: run("git", ["rev-parse", "HEAD"], braidRoot),
  ...
  model_target: { context: CONTEXT, reserved_boundary_tokens: 1,
    packing: "complete records only; task-local greedy packs; Z5PKV2 classes" },
  schedules: {
    C: "smooth deterministic task interleave; all answer weights 1",
    D: "byte-identical C packs; frozen equal-answer-mass task weights",
  },
  answer_weights: {
    C: { claim: 1, cloze: 1, retrieval: 1 },
    D: manifest.training.balancedAnswerWeights,
  },
  outputs,
  answer_spans: {
