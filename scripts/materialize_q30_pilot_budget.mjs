...
const TEMPLATE = "benchmarks/zero4-q30-v1/pilot-budget.json";
...
export function materialize(template, options, contractSha256) {
  ...
  assert.match(options["approval-id"], /^q30-[a-z0-9-]+$/);
...
function selfTest() {
  ...
  const runtime = materialize(template, {
    ...
    "approval-id": "q30-test-one-shot",
    "approved-by": "atimics",
    "approved-at": "2026-08-10T00:00:00Z",
  }, "b".repeat(64));
  assert.equal(runtime.authorization.maximum_optimizer_updates, 200);
  assert.equal(runtime.authorization.maximum_quantity_compute_usd, 0.5);
  assert.equal(runtime.authorization.contract_sha256, "b".repeat(64));
  ...
  console.log("Q3.0 runtime budget materializer self-test passed");
}
...
function main() {
  ...
  console.log(`wrote one-shot Q3.0 runtime budget to ${options.out}`);
}
