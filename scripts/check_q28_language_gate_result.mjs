...
export function validateEnvelope(launch, status, result, budget) {
  assert.equal(launch?.schema, "zero.q28_language_gate_launch.v1");
  assert.equal(launch.experiment, "zero4-q28-seed2-language-gate-v1");
  ...
  assert.equal(budget.candidate.id, "zero4-q28-seed2-u200");
  ...
  assert.equal(status?.schema, "zero.q28_language_gate_status.v1");
...
function selfTest() {
  ...
  console.log("Q2.8 language-gate result envelope self-test passed");
}
...
if (isMain) {
  if (process.argv.includes("--self-test")) {
    ...
    assert.equal(launch.workload_sha256,
      sha256("scripts/aws/q28-language-gate.sh"));
    assert.equal(launch.user_data_sha256,
      sha256("scripts/aws/q28-language-gate-user-data.sh"));
    validateContract(contract);
    validateBudget(budget, sha256(CONTRACT), result.model);
    validateResult(result, contract);
    validateEnvelope(launch, status, result, budget);
    console.log("OK Q2.8 candidate-bound language-gate result");
  }
}
