#!/usr/bin/env node

import assert from "node:assert/strict";
import crypto from "node:crypto";
import fs from "node:fs";
import path from "node:path";
import { spawnSync } from "node:child_process";

const PROFILE = "benchmarks/zero4-q28-v1/audit/profile.tsv";
const ACTIVATION = "benchmarks/zero4-q29-v1/activation-contract.json";
const PROFILE_SHA256 =
  "de858b2cddc21cacb25a831e63ab30eba2e4ea9e944b3010d8ba6759653a0e4e";
const MAXIMUM_UPDATES = 100;
const MEASUREMENT_UPDATES = Object.freeze([0, 25, 50, 75, 100]);
const MAXIMUM_QUANTITY_COMPUTE_USD = 0.25;
const CONDITIONAL_LANGUAGE_GATE_USD = 0.12;
const MINIMUM_QUANTITY_IMPROVEMENT = 0.8;
const MAXIMUM_REPLAY_REGRESSION = 0.0075;
const STOP_ORDER = Object.freeze([
  "replay guard", "quantity first hit", "update cap",
]);

function fail(message) { throw new Error(message); }
function sha256(file) {
  return crypto.createHash("sha256").update(fs.readFileSync(file)).digest("hex");
}
function readJson(file) { return JSON.parse(fs.readFileSync(file, "utf8")); }
function atomicJson(file, value) {
  const temporary = `${file}.tmp`;
  fs.writeFileSync(temporary, `${JSON.stringify(value, null, 2)}\n`);
  fs.renameSync(temporary, file);
}
function consumeAuthorization(file, authorizationSha256, sourceCommit, out) {
  const marker = `${file}.consumed`;
  fs.writeFileSync(marker, `${JSON.stringify({
    schema: "zero.q29_pilot_authorization_consumption.v1",
    authorization_sha256: authorizationSha256,
    source_commit: sourceCommit,
    output: out,
  }, null, 2)}\n`, { flag: "wx" });
  return marker;
}
function run(command, args, { quiet = false } = {}) {
  const result = spawnSync(command, args, {
    encoding: "utf8",
    maxBuffer: 64 * 1024 * 1024,
  });
  if (!quiet && result.stdout) process.stdout.write(result.stdout);
  if (result.stderr) process.stderr.write(result.stderr);
  if (result.status !== 0) fail(`${command} exited ${result.status}`);
  return result.stdout;
}
export function validateAuthorization(budget, sourceCommit) {
  assert.equal(budget?.schema, "zero.q29_conservative_exposure_pilot_budget.v1");
  assert.equal(budget.id, "zero4-q29-seed2-pilot-v1");
  assert.equal(budget.status, "run_authorized");
  assert.equal(budget.profile.path, PROFILE);
  assert.equal(budget.profile.sha256, PROFILE_SHA256);
  assert.equal(budget.proposed.diagnostic_seed, 2);
  assert.equal(budget.proposed.maximum_optimizer_updates, MAXIMUM_UPDATES);
  assert.deepEqual(budget.proposed.checkpoint_updates, MEASUREMENT_UPDATES);
  assert.equal(budget.proposed.quantity_improvement_minimum,
    MINIMUM_QUANTITY_IMPROVEMENT);
  assert.equal(budget.proposed.replay_regression_maximum,
    MAXIMUM_REPLAY_REGRESSION);
  assert.deepEqual(budget.proposed.stop_order, STOP_ORDER);
  assert.equal(
    budget.proposed.maximum_quantity_compute_usd,
    MAXIMUM_QUANTITY_COMPUTE_USD,
  );
  assert.equal(
    budget.proposed.conditional_language_gate_usd,
    CONDITIONAL_LANGUAGE_GATE_USD,
  );
  assert.equal(
    budget.proposed.language_gate_condition,
    "one first-hit candidate is hash-frozen",
  );
  assert.equal(budget.proposed.promotion_authorized, false);
  const authorization = budget.authorization;
  assert.equal(authorization.authorized, true);
  assert.equal(authorization.one_execution_only, true);
  assert.match(authorization.approval_id, /^q29-[a-z0-9-]+$/);
  assert.match(authorization.approved_by, /^[A-Za-z0-9_.-]+$/);
  assert.match(authorization.approved_at, /^\d{4}-\d{2}-\d{2}T/);
  assert.match(authorization.source_commit, /^[0-9a-f]{40}$/);
  assert.equal(authorization.source_commit, sourceCommit);
  assert.equal(authorization.profile_sha256, PROFILE_SHA256);
  assert.equal(authorization.maximum_optimizer_updates, MAXIMUM_UPDATES);
  assert.equal(
    authorization.maximum_quantity_compute_usd,
    MAXIMUM_QUANTITY_COMPUTE_USD,
  );
  assert.equal(authorization.language_gate_authorized, false);
  assert.equal(authorization.promotion_authorized, false);
  return budget;
}

export function validateInputLocks(activation) {
  assert.equal(activation?.schema,
    "zero.zero4_q29_activation_contract.v1");
  assert.equal(activation.lineage.implementation_issue, 83);
  assert.equal(activation.lineage.base_commit,
    "5ccd73e7e5eb733cf37652953d99163e0209dd13");
  assert.equal(activation.lineage.profile.path, PROFILE);
  assert.equal(activation.lineage.profile.sha256, PROFILE_SHA256);
  assert.equal(activation.mechanics.diagnostic_seed, 2);
  assert.equal(activation.mechanics.maximum_optimizer_updates,
    MAXIMUM_UPDATES);
  assert.deepEqual(activation.mechanics.measurement_updates,
    MEASUREMENT_UPDATES);
  assert.equal(activation.mechanics.quantity_improvement_minimum,
    MINIMUM_QUANTITY_IMPROVEMENT);
  assert.equal(activation.mechanics.replay_regression_maximum,
    MAXIMUM_REPLAY_REGRESSION);
  assert.deepEqual(activation.mechanics.stop_order, STOP_ORDER);
  assert.equal(activation.mechanics.resume_allowed, false);
  assert.equal(activation.mechanics.runtime_overrides_allowed, false);
  assert.deepEqual(activation.inputs.replay.map(({ path: file }) => file), [
    "corpus/bpe/zero-foundation.tok", "corpus/bpe/shakespeare.tok",
    "corpus/bpe/blake.tok", "corpus/bpe/crowley.tok",
    "corpus/bpe/bible-kjv.tok", "corpus/channel/literary-dialogue.tok",
  ]);
  for (const binding of [
    activation.lineage.contract, activation.lineage.profile,
    activation.lineage.initialization, activation.inputs.tokenizer,
    activation.inputs.quantity, ...activation.inputs.replay,
  ]) assert.equal(sha256(binding.path), binding.sha256,
    `${binding.path} source lock drifted`);
  return activation;
}

export function selectCandidate(measurements) {
  const actualUpdates = measurements.map(({ update }) => update);
  assert(actualUpdates.length >= 2 && actualUpdates.length <=
    MEASUREMENT_UPDATES.length);
  assert.deepEqual(actualUpdates,
    MEASUREMENT_UPDATES.slice(0, actualUpdates.length));
  const baseline = measurements[0];
  assert(Number.isFinite(baseline.quantity_training_loss) &&
    baseline.quantity_training_loss > 0);
  assert(Number.isFinite(baseline.replay_training_loss) &&
    baseline.replay_training_loss > 0);
  let terminalSeen = false;
  const candidates = measurements.slice(1).map((measurement) => {
    const quantityImprovement =
      (baseline.quantity_training_loss - measurement.quantity_training_loss) /
      baseline.quantity_training_loss;
    const replayRegression =
      (measurement.replay_training_loss - baseline.replay_training_loss) /
      baseline.replay_training_loss;
    let reason = "continue";
    if (replayRegression > MAXIMUM_REPLAY_REGRESSION) {
      reason = "replay-guard";
    } else if (quantityImprovement >= MINIMUM_QUANTITY_IMPROVEMENT) {
      reason = "first-hit";
    } else if (measurement.update === MAXIMUM_UPDATES) {
      reason = "update-cap";
    }
    assert.equal(terminalSeen, false,
      "measurements continued after a terminal stop decision");
    if (reason !== "continue") terminalSeen = true;
    return {
      ...measurement,
      quantity_improvement: quantityImprovement,
      replay_regression: replayRegression,
      feasible: reason === "first-hit",
      reason,
    };
  });
  assert(terminalSeen, "measurement sequence lacks a terminal stop decision");
  const terminal = candidates.at(-1);
  assert.notEqual(terminal.reason, "continue");
  return {
    baseline,
    candidates,
    selected: terminal.reason === "first-hit" ? terminal : null,
    stop_reason: terminal.reason,
  };
}

function parseArgs(argv) {
  if (argv.includes("--self-test")) return { selfTest: true };
  const options = { authorization: null, out: null,
    mechanics: "./conservative_exposure_pilot" };
  for (let index = 2; index < argv.length; ++index) {
    const option = argv[index];
    if (!option.startsWith("--") || index + 1 >= argv.length) {
      fail(`unknown or incomplete option ${option}`);
    }
    const key = option.slice(2);
    assert(Object.hasOwn(options, key), `unknown option ${option}`);
    options[key] = argv[++index];
  }
  assert(options.authorization, "pilot requires --authorization");
  assert(options.out, "pilot requires --out");
  return options;
}

function selfTest() {
  const source = "a".repeat(40);
  const budget = {
    schema: "zero.q29_conservative_exposure_pilot_budget.v1",
    id: "zero4-q29-seed2-pilot-v1",
    status: "run_authorized",
    profile: { path: PROFILE, sha256: PROFILE_SHA256 },
    proposed: {
      source_commit: "exact merged activation commit",
      diagnostic_seed: 2,
      maximum_optimizer_updates: 100,
      checkpoint_updates: [0, 25, 50, 75, 100],
      quantity_improvement_minimum: 0.8,
      replay_regression_maximum: 0.0075,
      stop_order: [...STOP_ORDER],
      maximum_quantity_compute_usd: 0.25,
      conditional_language_gate_usd: 0.12,
      language_gate_condition:
        "one first-hit candidate is hash-frozen",
      promotion_authorized: false,
    },
    authorization: {
      authorized: true,
      one_execution_only: true,
      approval_id: "q29-test-approval",
      approved_by: "atimics",
      approved_at: "2026-08-09T00:00:00Z",
      source_commit: source,
      profile_sha256: PROFILE_SHA256,
      maximum_optimizer_updates: 100,
      maximum_quantity_compute_usd: 0.25,
      language_gate_authorized: false,
      promotion_authorized: false,
    },
  };
  validateAuthorization(budget, source);
  for (const mutate of [
    (copy) => { copy.status = "proposal_not_authorized"; },
    (copy) => { copy.proposed.maximum_optimizer_updates = 101; },
    (copy) => { copy.proposed.checkpoint_updates = [0, 50, 100]; },
    (copy) => { copy.proposed.replay_regression_maximum = 0.008; },
    (copy) => { copy.authorization.authorized = false; },
    (copy) => { copy.authorization.source_commit = "b".repeat(40); },
    (copy) => { copy.authorization.maximum_quantity_compute_usd = 0.26; },
    (copy) => { copy.authorization.language_gate_authorized = true; },
    (copy) => { copy.authorization.promotion_authorized = true; },
  ]) {
    const copy = structuredClone(budget);
    mutate(copy);
    assert.throws(() => validateAuthorization(copy, source));
  }
  const selection = selectCandidate([
    { update: 0, quantity_training_loss: 10, replay_training_loss: 2 },
    { update: 25, quantity_training_loss: 2, replay_training_loss: 2.01 },
  ]);
  assert.equal(selection.selected.update, 25);
  assert.equal(selection.stop_reason, "first-hit");
  const replayGuard = selectCandidate([
    { update: 0, quantity_training_loss: 10, replay_training_loss: 2 },
    { update: 25, quantity_training_loss: 1, replay_training_loss: 2.016 },
  ]);
  assert.equal(replayGuard.selected, null);
  assert.equal(replayGuard.stop_reason, "replay-guard");
  const updateCap = selectCandidate([
    { update: 0, quantity_training_loss: 10, replay_training_loss: 2 },
    { update: 25, quantity_training_loss: 9, replay_training_loss: 2 },
    { update: 50, quantity_training_loss: 8, replay_training_loss: 2 },
    { update: 75, quantity_training_loss: 7, replay_training_loss: 2 },
    { update: 100, quantity_training_loss: 6, replay_training_loss: 2 },
  ]);
  assert.equal(updateCap.selected, null);
  assert.equal(updateCap.stop_reason, "update-cap");
  assert.throws(() => selectCandidate([
    { update: 0, quantity_training_loss: 2, replay_training_loss: 1 },
    { update: 50, quantity_training_loss: 1.9, replay_training_loss: 1 },
  ]));
  console.log("Q2.9 pilot authorization and selection self-test passed");
}

function main() {
  const options = parseArgs(process.argv);
  if (options.selfTest) return selfTest();
  const sourceCommit = run("git", ["rev-parse", "HEAD"], { quiet: true }).trim();
  const budget = validateAuthorization(readJson(options.authorization),
    sourceCommit);
  validateInputLocks(readJson(ACTIVATION));
  assert(!fs.existsSync(options.out), "pilot output already exists");
  const authorizationSha256 = sha256(options.authorization);
  const consumptionMarker = consumeAuthorization(options.authorization,
    authorizationSha256, sourceCommit, options.out);
  fs.mkdirSync(options.out, { recursive: false });
  const eventsPath = path.join(options.out, "events.jsonl");
  run(options.mechanics, [
    "--out-prefix", path.join(options.out, "checkpoint"),
    "--events", eventsPath,
    "--authorization-sha256", authorizationSha256,
  ]);
  const events = fs.readFileSync(eventsPath, "utf8").trim().split("\n")
    .map(JSON.parse);
  assert(events.every(({ schema }) => schema ===
    "zero.zero4_q29_pilot_event.v1"));
  const updateEvents = events.filter(({ type }) => type === "update");
  const complete = events.filter(({ type }) => type === "complete");
  assert.equal(complete.length, 1);
  assert(Number.isInteger(complete[0].updates_committed));
  assert(complete[0].updates_committed >= 25 &&
    complete[0].updates_committed <= MAXIMUM_UPDATES);
  assert.equal(complete[0].updates_committed % 25, 0);
  assert.equal(updateEvents.length, complete[0].updates_committed);
  assert.deepEqual(updateEvents.map(({ update }) => update),
    Array.from({ length: complete[0].updates_committed }, (_, index) =>
      index + 1));
  assert.equal(complete[0].language_gate_run, false);
  assert.equal(complete[0].promotion_run, false);
  const measurements = events.filter(({ type }) => type === "measurement")
    .map(({ update, quantity_training_loss, replay_training_loss,
      learned_state_digest }) => ({
      update, quantity_training_loss, replay_training_loss,
      learned_state_digest,
    }));
  const selection = selectCandidate(measurements);
  assert.equal(complete[0].updates_committed, measurements.at(-1).update);
  assert.equal(complete[0].stop_reason, selection.stop_reason);
  assert.equal(complete[0].candidate_checkpoint_available,
    selection.selected !== null);
  const stopChecks = events.filter(({ type }) => type === "stop-check");
  assert.equal(stopChecks.length, selection.candidates.length);
  for (let index = 0; index < stopChecks.length; ++index) {
    assert.equal(stopChecks[index].update, selection.candidates[index].update);
    assert.equal(stopChecks[index].reason, selection.candidates[index].reason);
    assert(Math.abs(stopChecks[index].quantity_improvement -
      selection.candidates[index].quantity_improvement) < 1e-12);
    assert(Math.abs(stopChecks[index].replay_regression -
      selection.candidates[index].replay_regression) < 1e-12);
  }
  let candidate = null;
  if (selection.selected) {
    const source = path.join(options.out,
      `checkpoint-u${String(selection.selected.update).padStart(6, "0")}.ckpt`);
    const target = path.join(options.out, "candidate.ckpt");
    fs.copyFileSync(source, target, fs.constants.COPYFILE_EXCL);
    candidate = {
      update: selection.selected.update,
      path: target,
      sha256: sha256(target),
      quantity_improvement: selection.selected.quantity_improvement,
      replay_regression: selection.selected.replay_regression,
    };
  }
  const result = {
    schema: "zero.zero4_q29_pilot_result.v1",
    source_commit: sourceCommit,
    authorization_sha256: authorizationSha256,
    authorization_consumption_marker: consumptionMarker,
    profile_sha256: PROFILE_SHA256,
    seed: 2,
    maximum_optimizer_updates: MAXIMUM_UPDATES,
    measurement_updates: MEASUREMENT_UPDATES,
    updates_committed: complete[0].updates_committed,
    stop_reason: selection.stop_reason,
    measurements,
    selection,
    candidate,
    decision: candidate ? "candidate-frozen" : "no-go",
    language_gate: {
      eligible_for_separate_authorization: candidate !== null,
      authorized: false,
      executed: false,
      maximum_compute_usd: CONDITIONAL_LANGUAGE_GATE_USD,
    },
    promotion: { authorized: false, executed: false },
  };
  atomicJson(path.join(options.out, "result.json"), result);
  console.log(`Q2.9 pilot ${result.decision}; language gate not executed`);
}

main();
