...
export function validateTemplate(template, binding, { checkFiles = true } = {}) {
  assert.equal(template?.schema, "zero.q29_language_gate_budget_template.v1");
  assert.equal(template.id, "zero4-q29-seed2-language-gate-v1");
  ...
  assert.equal(binding?.schema,
    "zero.q29_language_gate_candidate_binding.v1");
  assert.equal(binding.id, "zero4-q29-seed2-u50");
  assert.equal(binding.activation_commit,
    "c4f682c020e17d6231b6cab5542172f8ef6c1b76");
  assert.equal(binding.checkpoint.sha256,
    "b996514dc947f195723aa8f2ba6a86f3cb937e2c06d5968e3e3f8f65b6e96f86");
  ...
  assert.equal(binding.quantized.update, 50);
  ...
  assert.equal(template.authorization.implementation_issue, 87);
...
function selfTest() {
  ...
  console.log("Q2.9 language-gate budget materializer self-test passed");
}
...
if (isMain) {
  if (process.argv.includes("--self-test")) {
    ...
    console.log(`Materialized Q2.9 language-gate budget at ${options.out}`);
  }
}
