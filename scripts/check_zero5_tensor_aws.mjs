...
assert.equal(contract.status, "authorized-retry");
...
assert.equal(contract.workload.updates_per_candidate, 10);
...
assert.equal(contract.execution.maximum_instance_seconds, 606);
...
assert.equal(contract.execution_amendment.failed_attempt_ec2_usd,
  0.035511111111);
assert.ok(contract.execution_amendment.failed_attempt_ec2_usd +
...
for (const value of ["--instance-type \"$instance_type\"",
  ...
  "--if-none-match '*'", "--dry-run", "maximum_retry_ec2_usd"]) {
