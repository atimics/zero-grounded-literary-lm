...
function parseArgs(argv) {
  ...
  const options = { prefix: "/tmp/zero4-q24-seed2", out: "benchmarks/zero4-q24-v1/seed2", data: "corpus/faculty/q22", steps: 1000, consolidationSteps: 400, batch: 2, seed: 2, recoveryEvery: 25, fullEvery: 100, sentinelReplayBatches: 12, fullReplayBatches: 48 };
  ...
  assert(options.seed === 2, "Q2.4 v1 permits diagnostic seed 2 only; seeds 1 and 3 are sealed");
  ...
  assert(options.recoveryEvery === 25 && options.fullEvery === 100, "Q2.4 recovery/full cadences are frozen at 25/100 committed updates");
  return options;
}
function checkpointProgress(file) {
  const data = fs.readFileSync(file);
  assert(data.length >= 80 && data.toString("ascii", 0, 8) === "ZEROLM2\0", `invalid checkpoint ${file}`);
  assert(data.readUInt32LE(8) === 4, `Q2.4 requires checkpoint v4: ${file}`);
...
function resultMarkdown(result) {
  ...
  const diagnostics = result.guardDiagnostics ? `\nCumulative guard decisions: ${result.guardDiagnostics.accepted} accepted, ${result.guardDiagnostics.rejected} rejected. The maximum candidate composite replay increase was ${(100 * result.guardDiagnostics.maxCumulativeRelativeIncrease).toFixed(3)}%; ${result.guardDiagnostics.warningExceedances} attempts exceeded the 1% warning band and ${result.guardDiagnostics.hardExceedances} exceeded the 1.5% hard ceiling.\n` : "";
  return `# ZERO.4-Q2.4 seed 2 cumulative guard\n\nDecision: **${result.decision}**. Stop: ${result.stoppedReason}.\n\n${promotion}${modelHash}${diagnostics}${lastFrontier ? `The last Pareto checkpoint's public replay regression was ${(100 * lastFrontier.replayRegression).toFixed(3)}%.\n\n` : ""}| Committed | Attempts | Phase | Quantity pass | Minimum gate margin | Replay regression | Feasible |\n| ---: | ---: | --- | :---: | ---: | ---: | :---: |\n${rows || "| — | — | — | — | — | — | — |"}\n`;
}
function writeResultArtifacts(result) {
  ...
  atomicWrite(path.join(options.out, "selection.json"), `${JSON.stringify({ schema: "zero.zero4_q24_selection.v1", seed: 2, stage: result.stage, decision: result.decision, stoppedReason: result.stoppedReason, attempts: result.attempts, committed: result.committed, guardBudget: result.guardBudget, ...(result.guardDiagnostics ? { guardDiagnostics: result.guardDiagnostics } : {}), frontier: result.frontier, selected: result.selected ?? null, promotion: result.promotion }, null, 2)}\n`);
  atomicWrite(path.join(options.out, "RESULTS.md"), resultMarkdown(result));
}
function selfTest() {
  const buffer = Buffer.alloc(80); buffer.write("ZEROLM2\0", 0, "ascii"); buffer.writeUInt32LE(4, 8); buffer.writeBigUInt64LE(7n, 48); buffer.writeBigUInt64LE(9n, 64); buffer.writeUInt32LE(2, 72); buffer.writeUInt32LE(3, 76);
  const file = "/tmp/zero4-q24-driver-self-test.ckpt"; fs.writeFileSync(file, buffer);
  const progress = checkpointProgress(file);
  assert(progress.committed === 7 && progress.attempts === 9 && progress.consecutiveRejections === 2 && progress.transactionMode === 3, "checkpoint progress self-test failed");
  ...
  const report = resultMarkdown({ stage: "cumulative-guard", decision: "no-go", stoppedReason: "test", promotion: { evaluatedOnceAtEnd: false, reason: "test" }, guardDiagnostics: { accepted: 2, rejected: 1, warningExceedances: 1, hardExceedances: 1, maxCumulativeRelativeIncrease: 0.016 }, frontier: [{ committed: 100, totalAttempts: 103, phase: "acquisition", quantityPass: false, minimumFacultyMargin: -0.1, replayRegression: 0.01, feasible: false }] });
  assert(report.includes("1.600%") && report.includes("1.000%"), "guard result report self-test failed");
  let rejected = false; try { parseArgs(["node", "script", "--seed", "1"]); } catch { rejected = true; }
  assert(rejected, "sealed seed self-test failed");
  console.log("Q2.4 driver self-test passed");
}

const options = parseArgs(process.argv);
if (options.selfTest) { selfTest(); process.exit(0); }
const contract = JSON.parse(fs.readFileSync("benchmarks/zero4-q24-v1/contract.json", "utf8"));
const tools = { lm: "./literary_lm", export: "./export_literary", quantity: "./quantity_request_eval", check: "scripts/check_zero4_q24.mjs" };
...
for (const file of [...Object.values(tools).filter((entry) => entry.startsWith("./")), ...Object.values(teachers), files.tokens, files.sentinel, files.public, files.promotion, "corpus/literary.bpe", "benchmarks/zero4-q24-v1/contract.json"]) assert(fs.existsSync(file), `required file missing: ${file}`);
...
function trainingArgs({ checkpoint, first, phase, phaseOffset, phaseTotal, attempts, requestWeight }) {
  ...
  args.push("--transaction-mode", "cumulative-guard", "--transaction-log", files.attempts, "--transaction-phase", phase, "--transaction-probe", String(contract.guard.probe_every_attempts), "--transaction-budget", String(guardBudget), "--transaction-max-rejections", String(contract.guard.maximum_consecutive_rejections));
  return args;
}
...
while (!stoppedReason) {
  ...
  if (committed > 0 && committed % options.fullEvery === 0 && !frontier.some((entry) => entry.committed === committed)) {
    ...
    event("full-evaluation", { ...candidate, paretoAdded: changed.added }); atomicWrite(path.join(options.out, "frontier.json"), `${JSON.stringify({ schema: "zero.zero4_q24_frontier.v1", frontier }, null, 2)}\n`);
...
run("node", [tools.check, "benchmarks/zero4-q24-v1/contract.json", files.attempts], { quiet: true });
...
const guardDiagnostics = {
  ...
  warningRelativeIncrease: contract.guard.warning_relative_increase,
  hardRelativeIncrease: guardBudget,
  warningExceedances: recordedAttempts.filter((attempt) => attempt.cumulative_relative_change > contract.guard.warning_relative_increase).length,
  hardExceedances: recordedAttempts.filter((attempt) => attempt.cumulative_relative_change > guardBudget).length,
  maxCumulativeRelativeIncrease: Math.max(...recordedAttempts.map((attempt) => attempt.cumulative_relative_change)),
  minCumulativeRelativeChange: Math.min(...recordedAttempts.map((attempt) => attempt.cumulative_relative_change)),
...
if (!feasible.length) {
  const result = { schema: "zero.zero4_q24_result.v1", seed: 2, stage: "cumulative-guard", decision: "no-go", stoppedReason, attempts: totalAttempts, committed, guardBudget, guardDiagnostics, contractSha256: sha256("benchmarks/zero4-q24-v1/contract.json"), immutable_teachers: contract.immutable_teachers, quantity_corpus: contract.quantity_corpus, frontier, selected: null, promotion: { evaluatedOnceAtEnd: false, reason: "no jointly feasible public checkpoint" } };
  writeResultArtifacts(result); event("complete", { decision: "no-go", stoppedReason }); console.error("Q2.4 seed 2 no-go; promotion remained sealed"); process.exit(0);
}
