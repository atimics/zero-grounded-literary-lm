...
export function validateTemplate(template, binding, { checkFiles = true } = {}) {
  assert.equal(template?.schema, "zero.q28_u100_language_gate_budget_template.v1");
  assert.equal(template.id, "zero4-q28-seed2-u100-language-diagnostic-v1");
  ...
  assert.equal(binding?.schema,
    "zero.q28_u100_language_gate_candidate_binding.v1");
  assert.equal(binding.id, "zero4-q28-seed2-u100");
  assert.equal(binding.activation_commit,
    "606e1ab6c9b449bdfe647d283a35127e29f5e309");
  assert.equal(binding.checkpoint.sha256,
    "b19b39d76c01071dd68e9883bf1c86aa759e96cf82cd9b854b7d45d7090047dc");
  ...
  assert.equal(binding.quantized.update, 100);
  ...
  assert.equal(template.workload.q28_no_go_revisable, false);
  assert.equal(template.workload.diagnostic_only, true);
  assert.equal(template.authorization.implementation_issue, 80);
...
function selfTest() {
  ...
  console.log("Q2.8 update-100 diagnostic budget materializer self-test passed");
}
...
if (isMain) {
  if (process.argv.includes("--self-test")) {
    ...
    console.log(`Materialized Q2.8 update-100 diagnostic budget at ${options.out}`);
  }
}
