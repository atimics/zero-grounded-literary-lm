...
function evaluateGates(candidate, baseline, gates) {
  const derived = {
  ...
  const checks = {
    ...
    atlas_retention: candidate.atlas_nats_per_token <=
      ...
      derived.atlas_relative_regression <=
  ...
  for (const task of ["claim", "retrieval"]) {
    ...
    checks[task + "_swap_consistency"] =
      derived.swap_improvement[task] >= gates.swap_improvement_minimum;
...
function selfTest() {
  const choice = (accuracy, position0, position1, swap, pairExact) => ({
    choice_accuracy: accuracy,
    position_0_accuracy: position0,
    position_1_accuracy: position1,
    swap_consistency_accuracy: swap,
    pair_exact_accuracy: pairExact,
  });
  const baseline = {
    combined_nats_per_token: 2,
    evidence_nats_per_token: 2,
    atlas_nats_per_token: 2.2786,
    anchor_nats_per_token: 3.6942,
    cloze: { teacher_forced_exact_accuracy: 0 },
    choice: {
      claim: choice(0.50, 0.49, 0.51, 0.20, 0.10),
      retrieval: choice(0.50, 0.49, 0.51, 0.20, 0.10),
    },
  };
  const candidate = {
    combined_nats_per_token: 1.75,
    evidence_nats_per_token: 2.1,
    atlas_nats_per_token: 2.3,
    anchor_nats_per_token: 3.7,
    cloze: { teacher_forced_exact_accuracy: 0.02 },
    choice: {
      claim: choice(0.57, 0.55, 0.59, 0.27, 0.15),
      retrieval: choice(0.58, 0.56, 0.60, 0.28, 0.16),
    },
  };
  const gates = {
    combined_relative_improvement_minimum: 0.1,
    ...
    swap_improvement_minimum: 0.05,
    pair_exact_improvement_minimum: 0.03,
  };
  assert.equal(evaluateGates(candidate, baseline, gates).eligible, true);
  candidate.choice.retrieval.choice_accuracy = 0.51;
  const failed = evaluateGates(candidate, baseline, gates);
  assert.equal(failed.eligible, false);
  assert.equal(failed.checks.retrieval_choice_accuracy, false);
  process.stdout.write("ZERO.5 C4.2 evaluator self-test passed\n");
}
...
try {
  ...
  if (contract.schema !== "zero.c42_experiment_contract.v1" ||
    ...
    fail("C4.2 evaluation contract is not frozen");
  }
  ...
  const vectorResultPath =
    contract.proposed_primary_training.math_backend_evidence.result;
  requireArtifact(vectorResultPath,
    contract.proposed_primary_training.math_backend_evidence.result_sha256,
    "vector math result");
  const vectorResult = JSON.parse(fs.readFileSync(vectorResultPath));
  if (contract.proposed_primary_training.math_backend !==
        "gnu-libmvec-tanh-exp" ||
      vectorResult.decision.eligible_to_promote_vector_math_default !== true ||
      vectorResult.gates.sealed_test_stayed_closed !== true) {
    fail("selected C4.2 math backend is not supported by the frozen result");
  }
  ...
  const files = {
    ...
    evidence: path.join(importDirectory,
    ...
    choice: Object.fromEntries(["claim", "retrieval"].map(task => [task,
      path.join(importDirectory,
        task + ".validation.span-choice-eval.bin")
    ])),
  };
  requireArtifact(files.validation, imported.outputs.validation,
    "combined validation packs");
  requireArtifact(files.evidence,
    imported.outputs.validation_tasks["evidence-bundle"],
    "evidence validation packs");
  requireArtifact(files.cloze, imported.outputs.cloze_completion,
    "cloze exact-match evaluation");
  for (const task of ["claim", "retrieval"]) {
    requireArtifact(files.choice[task], imported.outputs.span_choices[task],
      task + " span-choice evaluation");
  }
  if (process.argv.includes("--preflight-only")) {
    process.stdout.write(JSON.stringify({
      schema: "zero.c42_evaluator_preflight.v1",
  ...
  requireArtifact(tokenizer, contract.upstream.tokenizer_sha256, "tokenizer");
  ...
  const gateResult = evaluateGates(candidate, baseline, contract.gates);
  gateResult.checks.test_metrics_opened = imported.test.metrics_opened;
  const result = {
    schema: "zero.c42_validation_result.v1",
