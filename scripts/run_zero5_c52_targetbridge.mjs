  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("close", code => {
      ...
      if (timedOut) reject(new Error(
        `TargetBridge execution exceeded ${maximumSeconds} seconds`));
...
function parseLog(log) {
  ...
  const accounting = log.match(/TargetBridge sampling sequences=(\d+) compute-token-exposures=(\d+) auxiliary-events=(\d+) wraps=(\d+)/u);
  const seconds = log.match(/training time ([0-9.]+) seconds/u);
  if (!accounting || !seconds) fail("TargetBridge training accounting is absent");
...
async function selfTest() {
  const parsed = parseLog("update       10 lm 3.4 aux 2.1 aux-acc 0.5 " +
    ...
    "TargetBridge sampling sequences=37768 compute-token-exposures=19337216 " +
    "auxiliary-events=293606 wraps=0\ntraining time 1.25 seconds\n");
  assert.equal(parsed.accounting.auxiliary_events, 293606);
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "zero-c52-runner-"));
  try {
  ...
  process.stdout.write("ZERO.5 C5.2 TargetBridge runner self-test passed\n");
}
...
const contractPath = path.resolve(option("--contract",
  "benchmarks/zero5-c52-targetbridge-v1/contract.json"));
...
if (contract.schema !== "zero.c52_targetbridge_contract.v1" ||
  ...
  fail("TargetBridge training is not explicitly authorized");
if (contract.execution.venue !== "local Apple Silicon" ||
  ...
  fail("TargetBridge execution exceeds its local no-cost authorization");
if (contract.test.content_present !== false ||
    contract.test.metrics_opened !== false ||
    contract.claim_boundary.verified_state_target_loss !== true ||
    contract.claim_boundary.symbolic_serialization !== false)
  fail("TargetBridge claim or test boundary changed");
...
const out = path.resolve(option("--out", "build/zero5-c52-targetbridge-v1/run"));
const binary = path.resolve(option("--target-trainer", "./zero5_c51_target_lm"));
const baseTrainer = path.resolve(option("--trainer", "./zero5_c32_lm_vector_math"));
...
if (evaluatorPreflight.artifacts_verified !== true ||
  ...
  fail("TargetBridge evaluator preflight failed");
if (preflightOnly) {
  process.stdout.write(JSON.stringify({
    schema: "zero.c52_targetbridge_training_preflight.v1",
    contract_sha256: contractSha256, updates: contract.training.update_groups,
...
if (fs.existsSync(executionPath)) {
  ...
  fs.writeFileSync(executionPath, JSON.stringify({
    schema: "zero.c52_targetbridge_execution.v1",
...
try {
  if (!fs.existsSync(resultPath)) {
    ...
    if (parsed.reports.at(-1)?.update !== training.update_groups ||
      ...
      fail("TargetBridge completed with different accounting");
    ...
    run("node", [contract.implementation.evaluator,
      ...
      "--target-trainer", binary, "--trainer", baseTrainer,
    ...
    const result = { schema: "zero.c52_targetbridge_result.v1",
      experiment: contract.experiment, contract_sha256: contractSha256,
      status: validation.replication_eligible ? "go-for-replication-request" : "no-go",
      training: { ...parsed, best_checkpoint: artifact(bestCheckpoint),
        best_auxiliary_head: artifact(`${bestCheckpoint}.aux`), cost_usd: 0 },
