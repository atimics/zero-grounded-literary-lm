...
function requireArtifact(file, expected, label) {
  ...
  const expectedSha256 = typeof expected === "string" ? expected : expected.sha256;
  if (observed.sha256 !== expectedSha256 ||
      (typeof expected === "object" && expected.bytes !== undefined &&
       observed.bytes !== expected.bytes)) {
    fail(`${label} drifted from its frozen identity`);
  }
  return observed;
}
...
function selfTest() {
  const ceiling = 1.5;
  const hourly = 0.68;
  const maximumSeconds = Math.floor(ceiling * 3600 / hourly);
  if (maximumSeconds !== 7941 || maximumSeconds * hourly / 3600 > ceiling ||
      (maximumSeconds + 1) * hourly / 3600 <= ceiling) {
    fail("budget calculation self-test failed");
  }
  const accounting = "packed sampling sequences=37768 " +
    "compute-token-exposures=19337216 active-targets=12735377 " +
    "answer-targets=3018930 claim-answer-targets=731688 " +
    "cloze-answer-targets=135644 retrieval-answer-targets=2151598 " +
    "padding-targets=6601839 wraps=0";
  if (!parseAccounting(accounting)) fail("accounting self-test failed");
  process.stdout.write("ZERO.5 C4.2 runner self-test passed\n");
}
...
const contractPath = "benchmarks/zero5-c42-v1/contract.json";
const contractBytes = fs.readFileSync(contractPath);
const contract = JSON.parse(contractBytes);
const contractSha256 = sha256(contractBytes);
if (contract.schema !== "zero.c42_experiment_contract.v1" ||
    contract.status !== "authorized-unrun" || contract.authorized !== true ||
    contract.authorization?.approval_id !== "zero5-c42-aws-2026-08-28-v1") {
  fail("C4.2 training is not explicitly authorized");
}
for (const name of ["trainer", "importer", "evaluator", "runner"]) {
  requireArtifact(contract.implementation[name],
    contract.implementation[`${name}_sha256`], `frozen C4.2 ${name}`);
}

const importDirectory = path.resolve(option("--import-dir",
  "build/zero5-c42-v1/import-final"));
...
const out = path.resolve(option("--out", "build/zero5-c42-v1/run"));
const preflightOnly = process.argv.includes("--preflight-only");
const resumeRun = process.argv.includes("--resume-run");
const binary = "./zero5_c32_lm_vector_math";
const importedPath = path.join(importDirectory, "import.json");
const imported = JSON.parse(fs.readFileSync(importedPath));

requireArtifact(importedPath, contract.verified_import.manifest_sha256,
  "C4.2 import manifest");
if (imported.schema !== "zero.c42_import.v1" ||
    imported.release.id !== contract.upstream.release_id ||
    imported.test.content_present !== false || imported.test.parsed !== false ||
    imported.test.tokenized !== false || imported.test.packed !== false ||
    imported.test.metrics_opened !== false) {
  fail("C4.2 import or sealed-test policy changed");
}

const files = {
  tokenizer: path.join(c0Directory, "byte-bpe512.sero"),
  initial: path.join(c2Directory, "best.ckpt"),
  train: path.join(importDirectory, "train.primary.grouped.z5pack"),
  validation: path.join(importDirectory, "validation.z5pack"),
...
requireArtifact(files.tokenizer, contract.upstream.tokenizer_sha256, "tokenizer");
requireArtifact(files.initial, contract.initialization.checkpoint_sha256,
  "C2 checkpoint");
requireArtifact(files.train, imported.outputs.train_primary, "training packs");
requireArtifact(files.validation, imported.outputs.validation,
  "validation packs");
...
const evaluatorPreflight = JSON.parse(run("node", [
  contract.implementation.evaluator, "--import-dir", importDirectory,
  "--preflight-only",
]).trim());
if (evaluatorPreflight.evaluator_artifacts_verified !== true ||
    evaluatorPreflight.test_metrics_opened !== false) {
  fail("C4.2 evaluator preflight failed");
}
if (preflightOnly) {
  process.stdout.write(JSON.stringify({
    schema: "zero.c42_training_preflight.v1",
    contract_sha256: contractSha256,
    import_manifest_sha256: artifact(importedPath).sha256,
    updates: contract.proposed_primary_training.updates,
    maximum_batch_sequences:
      contract.proposed_primary_training.maximum_batch_sequences,
    maximum_total_ec2_usd: contract.execution.maximum_total_ec2_usd,
    evaluator_artifacts_verified: true,
    test_metrics_opened: false,
  }) + "\n");
  process.exit(0);
}
if (!fs.existsSync(binary)) fail(`vector trainer is missing: ${binary}`);
...
if (!fs.existsSync(resultPath)) {
  ...
  const training = contract.proposed_primary_training;
  ...
  await runStreaming(binary, [
    ...
    "--require-math-backend", training.math_backend,
    "--require-attention-backend", training.attention_backend,
    ...
    "--cosine", "--dropout", String(training.residual_dropout),
  ...
  if (reports.at(-1)?.update !== training.updates) {
    fail("C4.2 training did not reach its final update");
  }
  ...
  if (!accounting || accounting.pack_sequences !== expected.packs ||
      ...
      accounting.claim_answer_targets !== expected.answer_targets_by_task.claim ||
      accounting.cloze_answer_targets !== expected.answer_targets_by_task.cloze ||
      accounting.retrieval_answer_targets !==
        expected.answer_targets_by_task.retrieval || accounting.wraps !== 0) {
    fail("C4.2 training accounting changed");
  }
  ...
  fs.writeFileSync(resultPath, JSON.stringify({
    ...
    implementation: {
      ...
      math_backend: training.math_backend,
      attention_backend: training.attention_backend,
    },
    training: {
      completed_updates: finalReport.update,
    ...
    checkpoints: {
      active: artifact(activeCheckpoint), best: artifact(bestCheckpoint),
    },
    validation,
    decision: {
...
if (result.training.completed_updates !==
    contract.proposed_primary_training.updates ||
    result.decision.test_metrics_opened !== false) {
  fail("cached C4.2 result is incomplete or unsafe");
}
fs.writeFileSync(executionPath, JSON.stringify({
  schema: "zero.c42_execution.v1",
  experiment: contract.experiment,
