...
function checkContract(contract) {
  assert(contract.schema === "zero.zero4_q26_contract.v1", "wrong contract schema");
  ...
  assert(contract.guard.mode === "cumulative-tangent", "wrong guard mode");
  ...
  assert(contract.guard.projection.enabled === true, "tangent projection is not enabled");
  assert(contract.guard.projection.gradient === "arithmetic mean gradient of the same six frozen replay validation-window losses at the committed pre-attempt state", "projection gradient changed");
  assert(contract.guard.projection.scope === "one global Euclidean projection over all trainable weights", "projection scope changed");
  assert(contract.guard.projection.trigger === "project only when the replay-gradient dot candidate weight displacement is positive", "projection trigger changed");
  assert(contract.guard.projection.moment_policy === "retain the AdamW moments produced by the scaled trial; project weights only; restore weights and moments together before every retry or outer rejection", "projection moment policy changed");
...
function checkTrial(trial, attempt, contract, trialIndex) {
  ...
  for (const field of ["projection_applied", "projection_coefficient", "projection_pre_dot", "projection_post_dot", "projection_removed_fraction"]) assert(Object.hasOwn(trial, field), `trial missing ${field}`);
  assert(typeof trial.projection_applied === "boolean", "invalid projection flag");
  assert(finite(trial.projection_coefficient) && finite(trial.projection_pre_dot) && finite(trial.projection_post_dot) && finite(trial.projection_removed_fraction), "non-finite projection diagnostics");
  if (trial.projection_applied) {
    assert(trial.projection_pre_dot > 0 && trial.projection_coefficient > 0, "projection applied without a conflicting displacement");
    assert(Math.abs(trial.projection_post_dot) <= 1e-5 * (1 + Math.abs(trial.projection_pre_dot)), "projected displacement is not tangent");
    assert(trial.projection_removed_fraction > 0 && trial.projection_removed_fraction <= 1.00001, "invalid removed displacement fraction");
  } else {
    assert(trial.projection_pre_dot <= 0 && trial.projection_coefficient === 0 && trial.projection_removed_fraction === 0, "non-conflicting displacement was modified");
    assert(close(trial.projection_pre_dot, trial.projection_post_dot, 1e-12), "unprojected displacement changed");
  }
...
function checkAttempt(attempt, contract) {
  ...
  assert(attempt.mode === "cumulative-tangent", "invalid transaction mode");
  ...
  assert(finite(attempt.cumulative_replay_gradient_norm) && attempt.cumulative_replay_gradient_norm > 0, "invalid cumulative replay gradient norm");
  for (const field of ["projection_applied", "projection_coefficient", "projection_pre_dot", "projection_post_dot", "projection_removed_fraction"]) assert(attempt[field] === finalTrial[field] || (finite(attempt[field]) && finite(finalTrial[field]) && close(attempt[field], finalTrial[field], 1e-12)), `parent ${field} does not match selected trial`);
  if (attempt.decision === "accept") {
    ...
    assert(attempt.reason === (attempt.backtrack_trial_count === 1 ? "tangent-within-cumulative-replay-budget" : "tangent-backtracked-within-cumulative-replay-budget"), "accepted attempt reason is inconsistent");
  } else {
    ...
    assert(attempt.reason === "cumulative-tangent-exhausted", "rejected attempt reason is inconsistent");
  }
...
function trial(index, scale, candidate, decision) {
  return { index, scale, learning_rate: 0.1 * scale, candidate_mean: candidate, relative_change: candidate - 1, decision, projection_applied: true, projection_coefficient: 0.1 * scale, projection_pre_dot: 0.01 * scale, projection_post_dot: 0, projection_removed_fraction: 0.2, ranges: makeRanges(candidate) };
}
function baseAttempt(overrides = {}) {
  return { schema: "zero.optimizer_attempt.v4", attempt: 1, proposed_committed_update: 1, committed_update: 1, phase: "smoke", mode: "cumulative-tangent", source_mask: "0000000000000001", source_ids: [0], replay_range: 0, learning_rate: 0.05, base_learning_rate: 0.1, accepted_scale: 0.5, backtrack_trial_count: 2, backtrack_trials: [trial(0, 1, 1.02, "retry"), trial(1, 0.5, 1.01, "accept")], guard_budget: 0.015, functional_probe: true, probe_before: 1, probe_after: 1.01, relative_probe_change: 0.01, faculty_replay_gradient_cosine: -0.5, gradient_norm: 1, displacement_norm: 0.1, predicted_replay_drift: 0.2, fisher_weighted_drift: 0.3, cumulative_replay_gradient_norm: 2, projection_applied: true, projection_coefficient: 0.05, projection_pre_dot: 0.005, projection_post_dot: 0, projection_removed_fraction: 0.2, decision: "accept", reason: "tangent-backtracked-within-cumulative-replay-budget", rollback_digest: "0000000000000000", cumulative_probe_baseline: 1, cumulative_probe_after: 1.01, cumulative_relative_change: 0.01, cumulative_ranges: makeCumulativeRanges(1.01), groups: [{ id: "token_embedding", replay_drift: 0.2, displacement_norm: 0.1, fisher_weighted_drift: 0.3 }], ...overrides };
}
...
function selfTest() {
  ...
  const trials = contract.guard.trial_scales.map((scale, index) => trial(index, scale, 1.02, index === 7 ? "reject" : "retry"));
  const last = trials.at(-1);
  checkAttempt(baseAttempt({ committed_update: 0, learning_rate: 0.00078125, accepted_scale: null, backtrack_trial_count: 8, backtrack_trials: trials, projection_coefficient: last.projection_coefficient, projection_pre_dot: last.projection_pre_dot, projection_post_dot: last.projection_post_dot, projection_removed_fraction: last.projection_removed_fraction, decision: "reject", reason: "cumulative-tangent-exhausted", rollback_digest: "1234567890abcdef", cumulative_probe_after: 1.02, cumulative_relative_change: 0.02, cumulative_ranges: makeCumulativeRanges(1.02) }), contract);
  console.log("Q2.6 contract and tangent checker self-test passed");
}
...
const requireProjection = args.includes("--require-projection");
const paths = args.filter((argument) => !["--require-backtrack", "--require-full-scale", "--require-exhaustion", "--require-projection"].includes(argument));
const contractPath = paths[0] ?? "benchmarks/zero4-q26-v1/contract.json";
...
if (logPath) {
  ...
  if (requireProjection) assert(attempts.some((attempt) => attempt.decision === "accept" && attempt.projection_applied), "attempt log contains no projection-applied commit");
  console.log(`Q2.6 check: contract and ${lines.length} tangent optimizer attempts passed`);
} else {
  console.log("Q2.6 check: contract passed");
}
