...
export function validateEnvelope(launch, status, result, budget) {
  assert.equal(launch?.schema, "zero.q28_u100_language_gate_launch.v1");
  assert.equal(launch.experiment, "zero4-q28-seed2-u100-language-diagnostic-v1");
  ...
  assert.equal(budget.candidate.id, "zero4-q28-seed2-u100");
  ...
  assert.equal(budget.provenance.q28_no_go_revisable, false);
  assert.equal(budget.provenance.diagnostic_only, true);
  assert.equal(budget.authorization.one_execution_only, true);

  assert.equal(status?.schema, "zero.q28_u100_language_gate_status.v1");
...
function selfTest() {
  ...
  const budget = {
    ...
    id: "zero4-q28-seed2-u100-language-diagnostic-v1",
    status: "authorized",
    candidate: {
      id: "zero4-q28-seed2-u100",
  ...
  console.log("Q2.8 update-100 diagnostic result envelope self-test passed");
}
...
if (isMain) {
  if (process.argv.includes("--self-test")) {
    ...
    assert.equal(launch.workload_sha256,
      sha256("scripts/aws/q28-u100-language-gate.sh"));
    assert.equal(launch.user_data_sha256,
      sha256("scripts/aws/q28-u100-language-gate-user-data.sh"));
    validateContract(contract);
    validateBudget(budget, sha256(CONTRACT), result.model);
    validateResult(result, contract);
    validateEnvelope(launch, status, result, budget);
    console.log("OK Q2.8 update-100 diagnostic language-gate result");
  }
}
