...
async function selfTest() {
  const accounting = parseAccounting("packed sampling sequences=37768 " +
    ...
    "cloze-answer-targets=430769 retrieval-answer-targets=1470102 " +
...
if (contract.schema !== "zero.c43_experiment_contract.v1") {
  fail("unexpected C4.3 contract schema");
}
if (contract.status !== "authorized-unrun" || contract.authorized !== true ||
    contract.authorization?.training_authorized !== true) {
  fail("C4.3 primary training is frozen but not authorized");
}
if (contract.execution.venue === "local") {
  if (contract.training.paid_compute_authorized !== false ||
      contract.training.cost_ceiling_usd !== null) {
    fail("local C4.3 execution must not claim paid-compute authorization");
  }
} else if (contract.training.paid_compute_authorized !== true ||
...
const out = path.resolve(option("--out", "build/zero5-c43-v1/run"));
...
requireArtifact(importedPath, contract.verified_import.first_receipt_sha256,
  "C4.3 import receipt");
if (imported.schema !== "zero.c43_import_receipt.v1" ||
  ...
  fail("C4.3 import or test boundary changed");
}
...
const files = {
  ...
  validation: path.join(importDirectory, "frozen-validation",
    "validation.z5pack"),
...
requireArtifact(files.train, contract.verified_import.primary,
  "primary training packs");
requireArtifact(files.validation,
  contract.verified_import.frozen_validation.combined_sha256,
  "frozen validation packs");
...
if (evaluatorPreflight.evaluator_artifacts_verified !== true ||
  ...
  fail("C4.3 evaluator preflight failed");
}
if (preflightOnly) {
  process.stdout.write(JSON.stringify({
    schema: "zero.c43_training_preflight.v1",
    ...
    maximum_total_compute_usd: contract.training.cost_ceiling_usd,
...
if (fs.existsSync(executionPath)) {
  ...
  fs.writeFileSync(executionPath, JSON.stringify({
    schema: "zero.c43_execution.v1", experiment: contract.experiment,
    contract_sha256: contractSha256, status: "running",
  }, null, 2) + "\n");
}
...
if (!fs.existsSync(resultPath)) {
  ...
  if (reports.at(-1)?.update !== training.update_groups) {
    fail("C4.3 training did not reach its final update");
  }
  ...
  if (!accounting || accounting.pack_sequences !== expected.packs ||
    ...
    fail("C4.3 training accounting changed");
  }
  ...
  run("node", [contract.implementation.evaluator,
    ...
    "--import-dir", importDirectory, "--out", validationPath,
  ], environment);
  const validation = JSON.parse(fs.readFileSync(validationPath));
  fs.writeFileSync(resultPath, JSON.stringify({
    schema: "zero.c43_result.v1", experiment: contract.experiment,
    status: "complete", contract_sha256: contractSha256,
    ...
    validation,
    decision: { eligible_for_promotion: validation.eligible_for_promotion,
      test_metrics_opened: false },
...
if (result.training.completed_updates !== contract.training.update_groups ||
  ...
  fail("cached C4.3 result is incomplete or unsafe");
}
fs.writeFileSync(executionPath, JSON.stringify({
  schema: "zero.c43_execution.v1", experiment: contract.experiment,
  contract_sha256: contractSha256, status: "complete",
