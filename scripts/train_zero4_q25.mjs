...
function parseArgs(argv) {
  ...
  const options = { prefix: "/tmp/zero4-q25-seed2", out: "benchmarks/zero4-q25-v1/seed2", data: "corpus/faculty/q22", steps: 1000, consolidationSteps: 400, batch: 2, seed: 2, recoveryEvery: 25, fullEvery: 100, sentinelReplayBatches: 12, fullReplayBatches: 48 };
  ...
  assert(options.seed === 2, "Q2.5 v1 permits diagnostic seed 2 only; seeds 1 and 3 are sealed");
  ...
  assert(options.recoveryEvery === 25 && options.fullEvery === 100, "Q2.5 recovery/full cadences are frozen at 25/100 committed updates");
  return options;
}
function checkpointProgress(file) {
  const data = fs.readFileSync(file);
  assert(data.length >= 80 && data.toString("ascii", 0, 8) === "ZEROLM2\0", `invalid checkpoint ${file}`);
  assert(data.readUInt32LE(8) === 4, `Q2.5 requires checkpoint v4: ${file}`);
...
function writeResultArtifacts(result) {
  ...
  atomicWrite(path.join(options.out, "selection.json"), `${JSON.stringify({ schema: "zero.zero4_q25_selection.v1", seed: 2, stage: result.stage, decision: result.decision, stoppedReason: result.stoppedReason, attempts: result.attempts, committed: result.committed, guardBudget: result.guardBudget, guardDiagnostics: result.guardDiagnostics, frontier: result.frontier, selected: result.selected ?? null, promotion: result.promotion }, null, 2)}\n`);
  atomicWrite(path.join(options.out, "RESULTS.md"), resultMarkdown(result));
}
function selfTest() {
  const buffer = Buffer.alloc(80); buffer.write("ZEROLM2\0", 0, "ascii"); buffer.writeUInt32LE(4, 8); buffer.writeBigUInt64LE(7n, 48); buffer.writeBigUInt64LE(9n, 64); buffer.writeUInt32LE(2, 72); buffer.writeUInt32LE(4, 76);
  const file = "/tmp/zero4-q25-driver-self-test.ckpt"; fs.writeFileSync(file, buffer);
  const progress = checkpointProgress(file);
  assert(progress.committed === 7 && progress.attempts === 9 && progress.consecutiveRejections === 2 && progress.transactionMode === 4, "checkpoint progress self-test failed");
  ...
  console.log("Q2.5 driver self-test passed");
}

const options = parseArgs(process.argv);
if (options.selfTest) { selfTest(); process.exit(0); }
const contract = JSON.parse(fs.readFileSync("benchmarks/zero4-q25-v1/contract.json", "utf8"));
const tools = { lm: "./literary_lm", export: "./export_literary", quantity: "./quantity_request_eval", check: "scripts/check_zero4_q25.mjs" };
...
for (const file of [...Object.values(tools).filter((entry) => entry.startsWith("./")), ...Object.values(teachers), files.tokens, files.sentinel, files.public, files.promotion, "corpus/literary.bpe", "benchmarks/zero4-q25-v1/contract.json"]) assert(fs.existsSync(file), `required file missing: ${file}`);
...
while (!stoppedReason) {
  ...
  assert(progress.transactionMode === 4, "Q2.5 checkpoint transaction mode drifted");
  ...
  if (committed > 0 && committed % options.fullEvery === 0 && !frontier.some((entry) => entry.committed === committed)) {
    ...
    event("full-evaluation", { ...candidate, paretoAdded: changed.added }); atomicWrite(path.join(options.out, "frontier.json"), `${JSON.stringify({ schema: "zero.zero4_q25_frontier.v1", frontier }, null, 2)}\n`);
  ...
  if (executedAttempts < requestedAttempts || progress.consecutiveRejections >= contract.guard.maximum_consecutive_exhausted_attempts) stoppedReason = "backtracking exhaustion fallback ended the phase";
}
run("node", [tools.check, "benchmarks/zero4-q25-v1/contract.json", files.attempts], { quiet: true });
...
if (!feasible.length) {
  const result = { schema: "zero.zero4_q25_result.v1", seed: 2, stage: "cumulative-backtracking", decision: "no-go", stoppedReason, attempts: totalAttempts, committed, guardBudget, guardDiagnostics, contractSha256: sha256("benchmarks/zero4-q25-v1/contract.json"), immutable_teachers: contract.immutable_teachers, quantity_corpus: contract.quantity_corpus, frontier, selected: null, promotion: { evaluatedOnceAtEnd: false, reason: "no jointly feasible public checkpoint" } };
  writeResultArtifacts(result); event("complete", { decision: "no-go", stoppedReason }); console.error("Q2.5 seed 2 no-go; promotion remained sealed"); process.exit(0);
}
